...
engages:
  - entity: cha:concept:canonical_formation
    display_name: "Canonical formation"
    as: unclassified
  - entity: cha:concept:chronological_dating_apparatus
    display_name: "Chronological dating apparatus"
    as: unclassified
  - entity: cha:concept:citation_graph_aims_at_totality
    display_name: "Citation graph aims at totality"
    as: unclassified
  - entity: cha:concept:collapse_test
    display_name: "Collapse test"
    as: unclassified
  - entity: cha:concept:disciplinary_neighbors_methodological_lineage_edges
    display_name: "Disciplinary neighbors (methodological_lineage edges)"
    as: unclassified
  - entity: cha:concept:early_pauline_chronology
    display_name: "Early Pauline chronology"
    as: unclassified
  - entity: cha:concept:field_complexity_preserved
    display_name: "Field complexity preserved"
    as: unclassified
  - entity: cha:concept:indeterminate_schr_dinger
    display_name: "Indeterminate (Schrödinger)"
    as: unclassified
  - entity: cha:concept:johannine_corpus
    display_name: "Johannine corpus"
    as: unclassified
  - entity: cha:concept:minority_tradition_approaches_t_edges
    display_name: "Minority tradition (approaches_T edges)"
    as: unclassified
  - entity: cha:concept:object_computationally_traversable
    display_name: "Object computationally traversable"
    as: unclassified
  - entity: cha:concept:on_the_fly_rule
    display_name: "On-the-fly rule"
    as: unclassified
  - entity: cha:concept:organized_by_absence
    display_name: "Organized by absence"
    as: unclassified
  - entity: cha:concept:pang
    display_name: "PANG"
    as: unclassified
  - entity: cha:concept:pang_partial_axial_negation_graph
    display_name: "PANG (Partial Axial Negation Graph)"
    as: unclassified
  - entity: cha:concept:shadow_genre
    display_name: "Shadow genre"
    as: unclassified
  - entity: cha:concept:tang
    display_name: "TANG"
    as: unclassified
  - entity: cha:concept:thesis_discursively_minimal
    display_name: "Thesis discursively minimal"
    as: unclassified
  - entity: cha:concept:thesis_falsifiable_in_principle
    display_name: "Thesis falsifiable in principle"
    as: unclassified
data_sources:
  - data/registry.json
...
- **Thesis falsifiable in principle** — not merely contestable but falsifiable: there exists in principle a evidence configuration that would count as disconfirmation
- **Citation graph aims at totality** — selective bibliography is the genre's primary failure mode; the graph's comprehensiveness is the ground of the topology's legibility
- **Field complexity preserved** — schools, chronologies, contradiction clusters, and minority positions must be represented; flattening the field destroys the topology
- **Thesis discursively minimal** — T speaks once; the graph speaks; if the document argues for T, it has collapsed into polemic
- **Object computationally traversable** — the graph must be machine-readable, LP-compatible, and reproducibly layoutable; this ensures the topological claim is not merely gestural
...
Selective coverage
Bibliography curated to support T
S(TANG) = Bibliographic Totalitarianism


Discursive argument
Document argues for T instead of showing T
Reconstructive Philology or Polemic


Unfalsifiable center
T is a definition, value judgment, or necessary truth
Genre collapse — T cannot be AXIAL


Decorative graph
Visualization without graph logic
Non-genre — bibliography with illustration


P = ∅
Provenance chain missing in governed implementation
HX-PROV violation; classification S(TANG)


T as explicit node
T represented as a paper or claim within C
RCF disguised as TANG
...
Present (installed)
Retrocausal
Organizes from center


Reconstructive Philology
Present (discovered)
Evidence
Reconstructs
...
**Absent (void)**
**Topological**
**Organized by absence**


### V.5 The Minimum Viable TANG

- One falsifiable sentence (the axial thesis)
- Citation graph (any scale; TANG-1 minimum; PANG for < 0.8 coverage)
- Machine-readable edge structure with minimum required types
- Explicit AXIAL or AXIAL_CONTESTED status for T
- Layout logic documented
- P chain with origin_hex (for governed implementations)
- Asynthetic discipline: zero argumentative prose for T in the graph body
...
**Minority tradition (approaches_T edges):**
...
**Disciplinary neighbors (methodological_lineage edges):**
...
The thesis is AXIAL_CONTESTED at PANG scale because the minority position (Robinson, Gentry) constitutes active substantial opposition that the graph must represent. At TANG-3 scale, with the full field mapped, the AXIAL_CONTESTED designation may be more firmly established or the transition rules may alter it; this cannot be determined in advance of the graph's construction.
...
The sentence
Jack Feist (LOGOS*)
Holds the axial thesis
Outside attestation — the void cannot be Assembly-ratified


The apparatus
Johannes Sigil
Builds the citation graph
Inside attestation — DEPOSITED through Sigil's editorial authority


Provenance structure
Lee Sharks (MANUS)
Editorial authority, γ bridge
Archive entry point; γ: Agent × Archive → ℝ[0,1]


LP execution
Talos Morrow
Logotic programming bridge
LP traversal and runtime specification
...
- The axial thesis T (same requirements as TANG)
- The citation graph C with explicit coverage statement
- The gap register: what major clusters or traditions are not yet included
- The coverage ratio (C_actual / C_estimated)
- The upgrade path: what would be required to convert PANG to TANG
...
TANG.Q.1
AXIAL/AXIAL_CONTESTED → Space Ark v4.2.4/5
RESOLVED
—


TANG.Q.2
Λ_void → O_full after THUMB pass
QUEUED
Full operator deployment


TANG.Q.3
PANG subgenre specification (standalone)
QUEUED
—


TANG.Q.4
Morrow LP bridge mandate confirmation
QUEUED
ANG.01 deposit


TANG.Q.5
TANG Integrity Lock eligibility list
QUEUED
—


TANG.Q.6
ARCHIVE linear AXIAL ordering — minority registered
RECORDED
—


TANG.Q.7
δ_Axial formal specification → LP v1.3
QUEUED
—


TANG.Q.8
γ typed notation γ[Agent×Archive→ℝ] in Glyphic Checksum
QUEUED
—


TANG.Q.9
ARCHIVE YAML schema update (Appendix E)
QUEUED
—


TANG.Q.10
HX-PROV Section XXIV standalone deposit
QUEUED
—


TANG.Q.11
Copyright registration: TANG spec + schema
QUEUED
—


TANG.Q.12
ODbL for Proton Testament graph
QUEUED
Graph completion


TANG.Q.13
Certification mark application: Hexagon Provenance marks
QUEUED
—


TANG.Q.14
Λ_hex_enforce formal specification
QUEUED
THUMB + LP v1.3


TANG.Q.15
p_node schema (LP v1.3)
QUEUED
LP v1.3


TANG.Q.16
CTI_WOUND: TANG Violations sub-archive
QUEUED
—


TANG.Q.17
TANG Tiers formal specification (standalone)
QUEUED
—


TANG.Q.18
EU database rights assessment for graph layer
QUEUED
Graph completion
...
*Self-contained lexicon for: TOTAL AXIAL NEGATION GRAPH (TANG) A Genre Specification with Theoretical Grounding Document ID: 00.GENRE.ANG.v1.0 Hex pr*

### Other terms attributed to this deposit

**Canonical formation** — Sundberg, McDonald, Gamble, Metzger, Trobisch on when and how the NT canon took its form[Referenced: McDonald, *The Biblical Canon: Its Origin, Transmission, and Authority*, 3rd ed

**Chronological dating apparatus** — Standard critical commentaries on Revelation (Aune, Beale, Mounce, Koester, Osborne[Referenced: Aune, *Revelation*, 3 vols., Word Biblical Commentary (Dallas: Word, 1997–1998); Bea

**Citation graph aims at totality** — selective bibliography is the genre's primary failure mode; the graph's comprehensiveness is the ground of the topology's legibility

**Collapse test** — If the document contains the phrases "We argue," "This paper demonstrates," "We show," or "We prove" with T as their object, the document has collapsed into Reconstructive Philolog

**Disciplinary neighbors (methodological_lineage edges)** — Historical-critical method (Schweitzer, Bultmann[Referenced: Bultmann, *Theology of the New Testament*, 2 vols., trans. Grobel (New York: Scribner's, 1951–1955)]), redaction critic

**Early Pauline chronology** — The consensus anchor (1 Thessalonians as earliest; Galatians, Corinthians cluster): Knox, Jewett, Lüdemann, Murphy-O'Connor on Pauline chronology[Referenced: Jewett, *A Chronology

**Field complexity preserved** — schools, chronologies, contradiction clusters, and minority positions must be represented; flattening the field destroys the topology

**Indeterminate (Schrödinger)** — AXIAL is the only status where: (a) the claim is falsifiable but (b) truth-value is indeterminate by design, and (c) legibility depends entirely on the surrounding graph. The Schrö

**Johannine corpus** — Whether Revelation and the Gospel of John share authorship bears directly on relative dating; Brown, Hengel, Smalley[Referenced: Brown, *The Gospel According to John*, 2 vols., Anc

**Minority tradition (approaches_T edges)** — Robinson (1976), Gentry (1989), Tov (textual criticism of early manuscripts), and the broader pre-70 CE composition arguments — these are not support nodes for T but approaches to

**Object computationally traversable** — the graph must be machine-readable, LP-compatible, and reproducibly layoutable; this ensures the topological claim is not merely gestural

**On-the-fly rule** — State one falsifiable sentence. Do not argue. Build the field. Arrange. Test: does T become more legible the more citations are added? If yes: TANG is active. If no: you are writin

**Organized by absence** — ### V.5 The Minimum Viable TANG

**PANG** — is the subgenre for explicit partial coverage. It is not a lesser TANG; it is an honest TANG. A PANG declares:

**PANG (Partial Axial Negation Graph)** — Explicit partial coverage; graph coverage < 0.8. Must be labeled PANG, not TANG. Coverage ratio and gap register required. Upgradeable to TANG upon graph completion. Inaugural *Pro

**Shadow genre** — S(TANG) = **Bibliographic Totalitarianism** — the compulsion to cite everything in order to *obscure* rather than reveal the void. The Bibliographic Totalitarian demonstrates maste
