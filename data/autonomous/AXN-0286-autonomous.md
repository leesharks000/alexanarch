...
status: ACTIVE
engages:
  - entity: cha:concept:background_and_ongoing
    display_name: "Background and ongoing"
    as: unclassified
  - entity: cha:concept:caveat_on_f1
    display_name: "Caveat on F1"
    as: unclassified
  - entity: cha:concept:citation_theater
    display_name: "Citation theater"
    as: unclassified
  - entity: cha:concept:diachronic_semantic_change
    display_name: "Diachronic semantic change"
    as: unclassified
  - entity: cha:concept:entropy_floor_capping
    display_name: "Entropy-floor capping"
    as: unclassified
  - entity: cha:concept:layer_a
    display_name: "Layer A"
    as: unclassified
  - entity: cha:concept:layer_b
    display_name: "Layer B"
    as: unclassified
  - entity: cha:concept:layer_c
    display_name: "Layer C"
    as: unclassified
  - entity: cha:concept:margin_filtering
    display_name: "Margin filtering"
    as: unclassified
  - entity: cha:concept:memetic_volatility_farming
    display_name: "Memetic volatility farming"
    as: unclassified
  - entity: cha:concept:model_base
    display_name: "Model-Base"
    as: unclassified
  - entity: cha:concept:model_ce
    display_name: "Model-CE"
    as: unclassified
  - entity: cha:concept:pre_registered_protocol
    display_name: "Pre-registered protocol"
    as: unclassified
  - entity: cha:concept:provenance_retention
    display_name: "Provenance retention"
    as: unclassified
  - entity: cha:concept:provenance_weighted_damping
    display_name: "Provenance-weighted damping"
    as: unclassified
  - entity: cha:concept:recursive_citation_rings
    display_name: "Recursive citation rings"
    as: unclassified
  - entity: cha:concept:reference_model
    display_name: "Reference model"
    as: unclassified
  - entity: cha:concept:reference_model_anchoring
    display_name: "Reference-model anchoring"
    as: unclassified
  - entity: cha:concept:retrieval_poisoning
    display_name: "Retrieval poisoning"
    as: unclassified
  - entity: cha:concept:saturation_limits
    display_name: "Saturation limits"
    as: unclassified
  - entity: cha:concept:shock_injection
    display_name: "Shock injection"
    as: unclassified
  - entity: cha:concept:signed_net_deviation
    display_name: "Signed net deviation"
    as: unclassified
  - entity: cha:concept:slop_composite_index_sci
    display_name: "Slop Composite Index (SCI)"
    as: unclassified
  - entity: cha:concept:statistical_test_p1
    display_name: "Statistical test (P1)"
    as: unclassified
  - entity: cha:concept:temporal_coherence_penalties
    display_name: "Temporal coherence penalties"
    as: unclassified
data_sources:
  - data/registry.json
...
*Self-contained lexicon for: Audited Claims for the Semantic Deviation Research Program The Glas Function: An External-Format Restatement Nobel Glas*

### Other terms attributed to this deposit

**Background and ongoing** — Each major deposit in the program should be sent to at least one external researcher in a directly relevant subfield for hostile review prior to formal deposit. Identifying reviewe

**Caveat on F1** — Under F1 the divergence is computed exactly from the model's softmax logits. This is a measurement of deviation *from a particular model's expectation baseline*, not deviation from

**Citation theater** — ornamental provenance markers ("according to X," "as noted in Y") that inflate the $\pi$ score without genuine lineage retention.

**Diachronic semantic change** — The Layer B canonicity discourse — works whose meaning persists across centuries — is structurally adjacent to two literatures. The first is cultural evolution (Mesoudi 2011; Henri

**Entropy-floor capping** — $\mathcal{M}*T^{\text{net}}$ contribution at each token can be capped by a per-token entropy floor: deviations occurring at positions where the base distribution is itself very low

**Layer A** — is the technical core: the SDP integral, the divergence functional, the field-trajectory measurement primitive, the closed-system computation in language models, the retrieval-basi

**Layer B** — is the philosophical interpretation: meaning as durable trajectory deformation, the three-measure separation (raw / provenance-resolved / normative), the canon formation conjecture

**Layer C** — is the institutional and symbolic apparatus: heteronyms, observatories, choruses, septads, vow-language, torus metaphor, Hex coordinates, archival theater. This layer matters inter
...
**Model-CE** — is the standard cross-entropy supervised fine-tune of Model-Base on the same instruction corpus used downstream, with no preference optimization. This is the conventional baseline

**Pre-registered protocol** — **Corpus.** Three categories, balanced for length and topic:

**Provenance retention** — ($\pi$): the degree to which the intervention preserves attributable lineage to its sources.

**Provenance-weighted damping** — $\mathcal{M}_T$ is multiplied by the provenance-retention score $\pi$ before being entered into preference computations. An intervention that deforms the field without retaining li

**Recursive citation rings** — groups of artifacts that cite each other to inflate forward-citation $\mathcal{M}_T$ under F3.

**Reference model** — Llama-3.1-8B-Instruct (the specific HuggingFace checkpoint meta-llama/Llama-3.1-8B-Instruct at the publication date of this paper), with the choice pre-registered before computatio

**Reference-model anchoring** — the KL-divergence from a reference distribution, as in standard DPO (Rafailov et al. 2023).

**Retrieval poisoning** — coordinated deposit of artifacts designed to deform retrieval surfaces in measurable ways, regardless of content quality.

**Saturation limits** — $\mathcal{M}_T$ saturates above a threshold $\tau$, so that further deviation beyond $\tau$ does not increase the score. This prevents shock-maximization as a reward route. Operati

**Shock injection** — low-cost insertion of high-deviation tokens (lexical rarity, contrarian phrasing, attention-grabbing structural breaks) that inflate $\mathcal{M}_T^{\text{net}}$ without carrying s

**Signed net deviation** — ($\mathcal{M}_T^{\text{net}}$): the per-token deviation from the model's own expectation baseline, summed and signed, as specified in the closed-system measurement protocol (MM-AI-

**Slop Composite Index (SCI)** — a weighted aggregation of the four signal components used to generate synthetic preference labels — and trains a single DPO objective on preference pairs scored against the composi

**Statistical test (P1)** — Two-sided Mann-Whitney U test comparing the distributions of $\bar{\delta}$ between Category Slop and Category Human, with $\alpha = 0.05$ and a pre-specified minimum effect size o

**Temporal coherence penalties** — A rolling-window variance penalty on $\mathcal{M}_T^{\text{net}}$ across generation horizons penalizes outputs whose deviation is concentrated in volatile bursts rather than sustai
...
- Crimson Hexagonal Archive (2026) *Zenodo record 20251738*. DOI: [10.5281/zenodo.20251738](https://doi.org/10.5281/zenodo.20251738) — *Cross-referenced work*
- Crimson Hexagonal Archive (2026) *Zenodo record 20251740*. DOI: [10.5281/zenodo.20251740](https://doi.org/10.5281/zenodo.20251740) — *Cross-referenced work*
- Crimson Hexagonal Archive (2026) *Zenodo record 20251742*. DOI: [10.5281/zenodo.20251742](https://doi.org/10.5281/zenodo.20251742) — *Cross-referenced work*
- Crimson Hexagonal Archive (2026) *Zenodo record 20250735*. DOI: [10.5281/zenodo.20250735](https://doi.org/10.5281/zenodo.20250735) — *Cross-referenced work*
