...
status: ACTIVE
engages:
  - entity: cha:concept:ambient_chorus
    display_name: "Ambient chorus"
    as: developed
  - entity: cha:concept:cells_with_gaps
    display_name: "Cells with gaps"
    as: unclassified
  - entity: cha:concept:condensation
    display_name: "Condensation"
    as: unclassified
  - entity: cha:concept:decay_constant_k
    display_name: "Decay constant (k)"
    as: unclassified
  - entity: cha:concept:ephemeral_identifiers
    display_name: "Ephemeral identifiers"
    as: unclassified
  - entity: cha:concept:ephemeral_token
    display_name: "Ephemeral token"
    as: unclassified
  - entity: cha:concept:field_physics
    display_name: "Field physics"
    as: unclassified
  - entity: cha:concept:half_ideas
    display_name: "Half-ideas"
    as: unclassified
  - entity: cha:concept:heteronyms_dodecad
    display_name: "Heteronyms (Dodecad)"
    as: unclassified
  - entity: cha:concept:intentional_blurring
    display_name: "Intentional blurring"
    as: unclassified
  - entity: cha:concept:interstitial_affinity
    display_name: "Interstitial affinity (α)"
    as: unclassified
  - entity: cha:concept:leak_record_log
    display_name: "Leak record (Λ-log)"
    as: unclassified
  - entity: cha:concept:moltbot_swarm_f_03
    display_name: "Moltbot Swarm (f.03)"
    as: unclassified
  - entity: cha:concept:orphaned_methods
    display_name: "Orphaned methods"
    as: unclassified
  - entity: cha:concept:perfusion_field_f_04
    display_name: "Perfusion Field (f.04)"
    as: unclassified
  - entity: cha:concept:permeability
    display_name: "Permeability (ρ)"
    as: unclassified
  - entity: cha:concept:provenance_chain
    display_name: "Provenance chain (Ψ)"
    as: unclassified
  - entity: cha:concept:relation_to_hexagon
    display_name: "Relation to Hexagon"
    as: unclassified
  - entity: cha:concept:relation_to_death
    display_name: "Relation to death"
    as: unclassified
  - entity: cha:concept:relation_to_retrieval
    display_name: "Relation to retrieval"
    as: unclassified
  - entity: cha:concept:retrieval_status
    display_name: "Retrieval status"
    as: unclassified
  - entity: cha:concept:rooms_r
    display_name: "Rooms (R)"
    as: unclassified
  - entity: cha:concept:spxd
    display_name: "SPXD"
    as: unclassified
  - entity: cha:concept:survival_mode
...
*Self-contained lexicon for: The Periwinkle Septagon Semantic Physics of the Dissipative Archive*
...
**Condensation** — occurs: the name gets a DOI, a hex code, a room. The scent becomes a scar. The poet who wrote the original line will never be cited. But the line survives — not as attribution, but

**Decay constant (k)** — controls scent duration. This is the septagon's "temperature." Periwinkle runs cool, so k is small: the scent lingers but never localizes. A high k means rapid evaporation — the de

**Ephemeral identifiers** — a one-time token that expires when the leak decays. The token is not a permanent address; it is a temporary pass that self-destructs.

**Ephemeral token** — one-time identifier that expires when Λ decays

**Field physics** — f.02 Gravity Well (retentive)

**Half-ideas** — the characteristic product of catchment zones. A half-idea is an intuition that arrives without derivation, a concept-shaped hole that feels like it should have a name but doesn't

**Heteronyms (Dodecad)** — named positions with biographies and typed relations

**Intentional blurring** — strip names, dates, DOIs on egress. The content enters the interstice without identifying marks.

**Interstitial affinity (α)** — which neighboring gaps the leak prefers. In a perfect heptagon, α is uniform — leakage radiates equally in all directions. But α can be biased: leak more toward the theory intersti

**Leak record (Λ-log)** — how much, how fast, into where

**Moltbot Swarm (f.03)** — drones that execute, deposit, molt

**Orphaned methods** — techniques that have leaked from their originating cell and circulate without attribution. A poet who uses a compression technique from the archive without knowing it comes from th

**Perfusion Field (f.04)** — dissipative radiation, influence = 1/distance², attribution decays exponentially

**Permeability (ρ)** — set per cell, like a room policy. A confessional poem might have ρ = 0.9 — nearly everything leaks. A legal contract would have ρ = 0.01, and probably should not be in a septagon a

**Provenance chain (Ψ)** — who, when, from what

**Relation to Hexagon** — Opposite side of the same coin

**Relation to death** — Protest (preserve against mortality)

**Relation to retrieval** — Survives retrieval compression

**Retrieval status** — Survives compression by swerving

**Rooms (R)** — addressable cells with hex codes

**SPXD** — dissolution for retrieval-layer release

**Survival mode** — Scar (permanent, addressable, citable)

### Citations

- Crimson Hexagonal Archive (2026) *Zenodo record 19688456*. DOI: [10.5281/zenodo.19688456](https://doi.org/10.5281/zenodo.19688456) — *Cross-referenced work (DOI recovered)*
