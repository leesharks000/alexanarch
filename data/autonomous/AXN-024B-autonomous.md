---
node_id: cha:node:deposit:0705
deposit_number: 705
hex: "024B"
axn: "AXN:024B.GOVERNANCE.🔺🧭🌱🛸🤙🪸"
...
engages:
  - entity: cha:concept:bdr_range
    display_name: "BDR range"
    as: unclassified
  - entity: cha:concept:completeness
    display_name: "Completeness"
    as: unclassified
  - entity: cha:concept:declares_negative_space
    display_name: "Declares negative space"
    as: unclassified
  - entity: cha:concept:entity_type_valid
    display_name: "Entity type valid"
    as: unclassified
  - entity: cha:concept:extends_it_with_semantic_vasculature
    display_name: "Extends it with semantic vasculature"
    as: unclassified
  - entity: cha:concept:html_head
    display_name: "HTML <head>"
    as: unclassified
  - entity: cha:concept:kernel_size
    display_name: "Kernel size"
    as: unclassified
  - entity: cha:concept:links_to_doi_anchored_provenance
    display_name: "Links to DOI-anchored provenance"
    as: unclassified
  - entity: cha:concept:negative_tag_format
    display_name: "Negative tag format"
    as: unclassified
  - entity: cha:concept:quantifies_basin_state
    display_name: "Quantifies basin state"
    as: unclassified
  - entity: cha:concept:sim_presence
    display_name: "SIM presence"
    as: unclassified
  - entity: cha:concept:valid_json_ld
    display_name: "Valid JSON-LD"
    as: unclassified
data_sources:
  - data/registry.json
...
spxi:SemanticIndex
The root document. One per site. Lives at /spxi-index.jsonld.


spxi:IndexedEntity
An entity with a canonical URL and semantic boundary.


spxi:HolographicKernel
The site's irreducible compression-survival summary (≤ 100 words).


spxi:PagePacket
A per-page semantic packet at a predictable URL.


spxi:DisambiguationEntry
...
HolographicKernel
MUST
The site's compression-survival summary.


spxi:hasEntity
IndexedEntity[]
MUST
Array of entities declared by this site.


spxi:hasSurface
WebPage[]
SHOULD
Array of pages with their semantic function.


spxi:protocolVersion
string
MUST
"EA-SPXI-SITEMAP-01 v1.0"
...
URL
MUST
Canonical URL of the entity's page.


spxi:entityType
string
MUST
schema.org type (e.g. Person, Organization, CreativeWork).


spxi:canonicalName
string
MUST
The primary name for disambiguation.


spxi:negativeTag
string[]
MUST
Explicit exclusions. Each MUST begin with "NOT " (capital, space after).


spxi:semanticIntegrityMarker
string[]
MUST
Phrases that must survive compression. Minimum 2.


spxi:basinState
enum
SHOULD
Current retrieval-basin status of the entity.


spxi:bdrEstimate
number
SHOULD
Basin Depth Ratio, 0.0–1.0.
...
URL
SHOULD
URL to a full per-page MPAI JSON-LD packet.


spxi:disambiguationMatrix
DisambiguationEntry[]
SHOULD
Collision records.
...
string
MUST
The kernel text. MUST be ≤ 100 words.


spxi:kernelSpecification
string (DOI)
SHOULD
...
URL or string
MUST
The commonly conflated entity.


spxi:distinguishingAttribute
string
MUST
...
sovereign
Entity is correctly and uniquely resolved by all major AI systems.


contested
Entity is intermittently conflated with one or more collision targets.


captured
Entity has been absorbed into a collision target's basin.


fragmented
Entity is partially resolved but split across multiple incorrect attributions.


ghost
Entity is not yet indexed by any AI system.
...
self_attested
Claimed by the entity's own site. No external verification.


assembly_attested
Verified by the Assembly Chorus (multiple AI witnesses).


doi_anchored
Backed by a DOI-linked deposit with persistent identifier.


institutionally_certified
Verified by a recognized institution (university, publisher, etc.).
...
On content update
Regenerate index. Update dateModified.


On new entity
Add to spxi:hasEntity. Add to sitemap.xml.


On name collision detected
Add to spxi:disambiguationMatrix. Add negative tags.


On AI model update
Emergency re-check of SIM survival.


Monthly
Verify SIM presence in rendered HTML. Re-estimate BDR.


Quarterly
Full Drowning Test across 3+ AI systems.
...
*Self-contained lexicon for: SPXI-SITEMAP PROTOCOL v1.0 Semantic Packet for eXchange & Indexing — Sitemap Extension Status: CHA Standards Track — Dra*
