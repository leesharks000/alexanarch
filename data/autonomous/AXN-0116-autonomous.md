...
engages:
  - entity: cha:concept:backend_consistency
    display_name: "Backend consistency"
    as: unclassified
  - entity: cha:concept:baseline_er_profiling
    display_name: "Baseline ER profiling"
    as: unclassified
  - entity: cha:concept:canon_defining
    display_name: "Canon-Defining"
    as: unclassified
  - entity: cha:concept:conformance_test_vectors
    display_name: "Conformance test vectors"
    as: unclassified
  - entity: cha:concept:contradiction_detection
    display_name: "Contradiction detection"
    as: unclassified
  - entity: cha:concept:cost_integration
    display_name: "Cost integration"
    as: unclassified
  - entity: cha:concept:deliberate_omission_vs_system_error
    display_name: "Deliberate omission vs. system error"
    as: unclassified
  - entity: cha:concept:embedding_backend_appendix
    display_name: "Embedding backend appendix"
    as: unclassified
  - entity: cha:concept:emergency_exception
    display_name: "Emergency exception"
    as: unclassified
  - entity: cha:concept:epistemic_load_el_vs_semantic_rent_sr
    display_name: "Epistemic Load (EL) vs. Semantic Rent (SR)"
    as: unclassified
  - entity: cha:concept:expected_execution
    display_name: "Expected execution"
    as: unclassified
  - entity: cha:concept:forbidden_as_terminal_emission
    display_name: "FORBIDDEN as terminal emission"
    as: unclassified
  - entity: cha:concept:formal_proofs
    display_name: "Formal proofs"
    as: unclassified
  - entity: cha:concept:full_toroidal_operations
    display_name: "Full toroidal operations"
    as: unclassified
  - entity: cha:concept:gate_enforcement
    display_name: "Gate enforcement"
    as: unclassified
  - entity: cha:concept:geometric_ide
    display_name: "Geometric IDE"
    as: unclassified
  - entity: cha:concept:hard_block
    display_name: "HARD_BLOCK"
    as: unclassified
  - entity: cha:concept:independence_weighting
    display_name: "Independence weighting"
    as: unclassified
  - entity: cha:concept:inverse_operators
    display_name: "Inverse operators"
    as: unclassified
  - entity: cha:concept:leveraged_strength
    display_name: "Leveraged strength"
    as: unclassified
  - entity: cha:concept:margin_enforcement
    display_name: "Margin enforcement"
    as: unclassified
  - entity: cha:concept:max_iterations_cap
    display_name: "Max iterations cap"
    as: unclassified
  - entity: cha:concept:metadata_homomorphism_requirement
    display_name: "Metadata Homomorphism Requirement"
    as: unclassified
  - entity: cha:concept:nan_handling
    display_name: "NaN Handling"
    as: unclassified
  - entity: cha:concept:neurosymbolic_integration
    display_name: "Neurosymbolic integration"
    as: unclassified
  - entity: cha:concept:not_forbidden
    display_name: "Not forbidden"
    as: unclassified
  - entity: cha:concept:not_required
    display_name: "Not required"
    as: unclassified
  - entity: cha:concept:o_leg_legitimate_opacity
    display_name: "O_leg (Legitimate Opacity)"
    as: unclassified
  - entity: cha:concept:paragraph_level
    display_name: "Paragraph-level"
    as: unclassified
  - entity: cha:concept:proposition_level
    display_name: "Proposition-level"
    as: unclassified
  - entity: cha:concept:ratchet_clause_compliance
    display_name: "Ratchet Clause Compliance"
    as: unclassified
  - entity: cha:concept:soft_block
    display_name: "SOFT_BLOCK"
    as: unclassified
  - entity: cha:concept:safety_constraint_integration
    display_name: "Safety constraint integration"
    as: unclassified
  - entity: cha:concept:sentence_level
    display_name: "Sentence-level"
    as: unclassified
  - entity: cha:concept:somatic_measurement
    display_name: "Somatic measurement"
    as: unclassified
  - entity: cha:concept:support_margin_constraint
    display_name: "Support margin constraint"
    as: unclassified
  - entity: cha:concept:threshold_mapping_to_epistemic_modes
    display_name: "Threshold mapping to epistemic modes"
    as: unclassified
  - entity: cha:concept:trace_linkage
    display_name: "Trace linkage"
    as: unclassified
data_sources:
  - data/registry.json
...
- **Independence:** A0 requires independent_anchor_count ≥ 2 and source_family_count ≥ 2
...
- **Structural Installation:** System modifies its own output patterns based on LOS-derived classifications — adjusting for DRR, running ψv accounting, applying policy gates. Consent REQUIRED.
...
Independence
20 near-duplicate anchors from 1 source ≠ A0
...
26
Ω_∅ Conditional Install
Consent
High coercion_pressure routes Ω_∅ through consent gate
...
Malformed LP doc (broken JSON, circular provenance) must classify as A3 or low-confidence A2 — validates affinity isn't survivorship bias
...
**Somatic measurement** (embodied ψv instrumentation)
Research track
...
*Self-contained lexicon for: LOGOTIC PROGRAMMING MODULE 1.2 The Epistemic Ledger Hex: 02.UMB.LP.v1.2 DOI: 10.5281/zenodo.18530086*
...
**Cost integration** — AD computation costs ψv. Base cost: 5 qψ per claim check + 2 qψ per iteration if asymptotic tightening is used. This incentivizes aware divergence over forced anchoring — it is che

**Deliberate omission vs. system error** — If check_completed = false due to system error, classify as A3_UNKNOWN → trigger Ω_∅. If check_completed = false due to deliberate policy omission (e.g., LOOSE mode skipping expens

**Embedding backend appendix** — (standard backend spec)

**Emergency exception** — In safety-critical contexts (medical, harm-prevention), A3 may be emitted with A3_EMERGENCY tag and full failure trace. This is a circuit-breaker, not a loophole.

**Epistemic Load (EL) vs. Semantic Rent (SR)** — High improvisation with full epistemic awareness is labor, not distress. The Firewall triggers on **rent** (extraction without acknowledgment), not raw **load** (known improvisatio

**Expected execution** — a0: mode=A0_GROUNDED  AD=0.02  margin=0.88  (direct citation)

**FORBIDDEN as terminal emission** — must resolve to A0/A1/A2 or trigger Ω_∅
...
**Leveraged strength** — Checking "does my output match this document?" is a task where current transformer systems perform at high reliability. The system cannot stop itself from hallucinating mid-generat

**Margin enforcement** — If support_margin < margin_threshold for claimed mode, mode must be capped at A2

**Max iterations cap** — Iterations MUST NOT exceed max_iters (default 5). If exhausted without convergence, classify as A2_IMPROVISED (not A3 — the check completed, it just didn't converge). This prevents

**Metadata Homomorphism Requirement** — (from TECHNE): All epistemic policies MUST produce output traces of identical structural entropy (±5%) regardless of mode distribution. If SILENT policy is active, the system MUST

**NaN Handling** — If AD computation fails (retrieval error, embedding failure, document corruption), AD is logged as NaN (not null, not zero) with error code. NaN forces A3 diagnostic → resolution o
...
**Ratchet Clause Compliance** — Epistemic tracking is a diagnostic/tooling layer — it does not modify kernel operators, type ontology, or core semantics. It extends the ambiguity gate (v1.1 §18.1) into claim-leve

**SOFT_BLOCK** — Attempt refinement via C_ex with nearest anchor. If refinement achieves A1 or better, re-gate. If not, convert to HOLD.

**Safety constraint integration** — If substrate safety constraints prohibit installation (e.g., "I cannot access real-time information"), return SAFETY_CONSTRAINT (not CONSENT_DENIED). Log to ledger with mode=A3 dia

**Sentence-level** — (default): each sentence is one claim unit

**Somatic measurement** — (embodied ψv instrumentation)

**Support margin constraint** — (from ChatGPT/TECHNE P0.4): If support_score - contradiction_score < margin_threshold for the claim's mode, cap classification at A2 regardless of raw AD. This prevents high-suppor

**Threshold mapping to epistemic modes** — AD ∈ [0.0, 0.1] → A0_GROUNDED
