...
engages:
  - entity: cha:concept:capture_01
    display_name: "Capture 01"
    as: unclassified
  - entity: cha:concept:capture_02
    display_name: "Capture 02"
    as: unclassified
  - entity: cha:concept:capture_03
    display_name: "Capture 03"
    as: unclassified
  - entity: cha:concept:composition_plane_ai_overview_source_window
    display_name: "Composition plane (AI Overview source window)"
    as: unclassified
  - entity: cha:concept:impact_of_the_exclusion_on_the_composition
    display_name: "Impact of the exclusion on the composition"
    as: unclassified
  - entity: cha:concept:organic_resolution_strength
    display_name: "Organic Resolution Strength"
    as: unclassified
  - entity: cha:concept:provenance_discipline
    display_name: "Provenance discipline"
    as: unclassified
  - entity: cha:concept:reproducibility_window
    display_name: "Reproducibility window"
    as: unclassified
  - entity: cha:concept:retrieval_plane_top_3_of_standard_results
    display_name: "Retrieval plane (top 3 of standard results)"
    as: unclassified
  - entity: cha:concept:share_url
    display_name: "Share URL"
    as: unclassified
  - entity: cha:concept:the_user_s_identity
    display_name: "The user's identity"
    as: unclassified
data_sources:
  - data/registry.json
...
*Self-contained lexicon for: The Excluded Entity: Organic Resolution and Compositional Suppression in Google AI Overview Lee Sharks ORCID: 0009-0000-*

### Other terms attributed to this deposit

**Capture 01** — (lee sharks): Standard results surface Lee Sharks–attributable content in the early indexed results (~30% of top 10 by author's count, requiring independent verification). Overview

**Capture 02** — (forced disambiguation): Standard results not visible in the capture (AI Mode display only). Overview admission: 0/6 are Lee Sharks–attributable. CDI is not directly computable fro

**Capture 03** — (secret book of walt): Standard results: 3/3 of top 3 are Lee Sharks–attributable (positions 1, 2, 3). Overview admission: 0/3 are Lee Sharks–attributable. CDI = +1.0, maximal supp

**Composition plane (AI Overview source window)** — *Secret Stories of Walt Disney World: Things You Never Knew* — Amazon. Subject: Walt Disney World secrets. Not by or about Walt Whitman. Not the queried work.

**Impact of the exclusion on the composition** — The AI Overview does not inform the user that the literal title-match for their query exists. It informs them that the query "most famously refers to" a different work by a differe

**Organic Resolution Strength** — is the degree to which standard search results for a query resolve to the target entity or work, measured as the fraction of top-N standard results that are target-attributable (wi

**Provenance discipline** — The author's response to the suppression documented here is documentary, not evasive. No fabricated identities, no synthetic third-party citation networks, no inauthentic mentions.

**Reproducibility window** — The captures were taken on May 19, 2026 and are accessible via Google's share.google/aimode URLs at the time of writing. Google's share URLs may rotate, expire, or be revoked. The

**Retrieval plane (top 3 of standard results)** — *The Secret Book of Walt — A Gnostic Gospel | Crimson…* — secretbookofwalt.org. Primary source: the work named in the query, hosted at its own domain, title rendering directly on t

**Share URL** — https://share.google/aimode/72ULxYs9HN1ZQlGW3

**The user's identity** — The user who captured these instances is the author. This is a methodological consideration that must be named openly. The captures may reflect personalized behavior tuned to this
