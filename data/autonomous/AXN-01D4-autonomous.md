---
node_id: cha:node:deposit:0618
deposit_number: 618
hex: "01D4"
axn: "AXN:01D4.GOVERNANCE.∞🟠🌘🕛♾️🔵"
title: "THE HEXAGON INTERFACE CONSTITUTION System Specification for the Crimson Hexagonal Archive as Governed Commons"
creator: "Lee Sharks"
...
engages:
  - entity: cha:concept:assembly
    display_name: "ASSEMBLY"
    as: founded
  - entity: cha:concept:constitutional
    display_name: "CONSTITUTIONAL"
    as: unclassified
  - entity: cha:concept:constraint_generates
    display_name: "Constraint generates"
    as: unclassified
  - entity: cha:concept:oracle
    display_name: "ORACLE"
    as: unclassified
  - entity: cha:concept:read
    display_name: "READ"
    as: unclassified
  - entity: cha:concept:room_proposals
    display_name: "Room proposals"
    as: unclassified
  - entity: cha:concept:storage_layer
    display_name: "Storage layer"
    as: unclassified
  - entity: cha:concept:trace
    display_name: "TRACE"
    as: unclassified
  - entity: cha:concept:work
    display_name: "WORK"
    as: unclassified
  - entity: cha:concept:who_can_annotate
    display_name: "Who can annotate"
    as: unclassified
  - entity: cha:concept:who_can_deposit
    display_name: "Who can deposit"
    as: unclassified
  - entity: cha:concept:who_can_modify_core_content
    display_name: "Who can modify core content"
    as: unclassified
  - entity: cha:concept:who_can_propose
    display_name: "Who can propose"
    as: unclassified
  - entity: cha:concept:who_can_ratify
    display_name: "Who can ratify"
    as: unclassified
  - entity: cha:concept:who_can_read
    display_name: "Who can read"
    as: unclassified
  - entity: cha:concept:who_can_review
    display_name: "Who can review"
    as: unclassified
data_sources:
  - data/registry.json
...
*Self-contained lexicon for: THE HEXAGON INTERFACE CONSTITUTION System Specification for the Crimson Hexagonal Archive as Governed Commons*

### Founded (institutions, journals, rooms established here)

**ASSEMBLY** — Governance interface. The user sees the current witness roster, pending proposals, review queues, active witness actions, and governance status. The Assembly mode is where proposal

### Other terms attributed to this deposit

**CONSTITUTIONAL** — Sections I through IV, VI, and VIII: governing principles, first-class objects, status algebra, interface modes, participation thresholds, and the governance clause. These sections

**Constraint generates** — The rooms have rules. The Airlock has criteria. The status algebra enforces sequence. The LOS is mandatory. These constraints are the features, not the limitations. An interface th

**ORACLE** — Query-driven traversal. The user asks a question. The interface routes the question through the room graph using OP.ROUTE, lighting a path from the question to the rooms and docume

**READ** — Document-first encounter. Enter a room and find its texts — poetry, theory, narrative, scholarship. Documents render as readable objects, not as metadata in a sidebar. The reading

**Room proposals** — must satisfy the six hard rules from EA-ARK-01 v4.2.7 §XXXI: (1) physics, (2) operators, (3) adjacency to at least one existing room, (4) a governing constraint, (5) a name, and (6

**Storage layer** — Structured JSON files for the prototype phase. Supabase (PostgreSQL + auth + real-time) for the platform phase. Every object has: id, type, version, created_by, created_at, updated

**TRACE** — Provenance navigation. Select any object and follow its history: who created it, what it derives from, what derives from it, where it has been compressed or cited, what witness act

**WORK** — Synthesis canvas. The user drags documents from multiple rooms, applies operators, and produces new work. The canvas enforces room physics — an operator applied outside its home ro

**Who can annotate** — Any authenticated user. Annotations carry GENERATED status.

**Who can deposit** — MANUS approves PROVISIONAL objects for Zenodo deposit. Successful DOI issuance promotes to DEPOSITED (0.9). DEPOSITED is an archival fact — it means the object is publicly fixed wi

**Who can modify core content** — MANUS with Assembly quorum (≥4/7) and documented rationale. Core content modification — including editing room physics, renaming rooms, changing operators, altering relation types,

**Who can propose** — Any authenticated user. Proposals enter the Airlock with GENERATED status and must specify: the object type, room assignment, physics or relations, and a statement of bearing-cost

**Who can ratify** — The Assembly by quorum (≥4/7) votes to promote DEPOSITED objects to RATIFIED (1.0). RATIFIED is a governance judgment — it means the Assembly recognizes the object as canonical and
...
*29 subjunctive addresses point to concepts in this deposit — see `data/semantic-addresses.json` for full list.*
