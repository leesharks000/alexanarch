---
node_id: cha:node:deposit:0066
deposit_number: 66
hex: "021F"
axn: "AXN:021F.GOVERNANCE.🏗️👐🫶🛸🗂️✨"
title: 'HEXAGONAL LICENSING PROTOCOL Comprehensive Specification for Distributed Contributor Infrastructure Lee Sharks, MANUS (Tier 0) Crimson Hexagonal Archive Version 1.0 · April 2026 Hex: 06.SEI.ARCHIVE.PR'
creator: "Lee Sharks"
...
status: ACTIVE
engages:
  - entity: cha:concept:cambridge_schizoanalytica_review_csr
    display_name: "Cambridge Schizoanalytica Review (CSR)"
    as: unclassified
  - entity: cha:concept:close_analytical_engagement
    display_name: "Close analytical engagement"
    as: unclassified
  - entity: cha:concept:deposits_the_license_on_zenodo
    display_name: "Deposits the license on Zenodo"
    as: unclassified
  - entity: cha:concept:determines_scope
    display_name: "Determines scope"
    as: unclassified
  - entity: cha:concept:discovery_query_pattern
    display_name: "Discovery query pattern"
    as: unclassified
  - entity: cha:concept:first_session_flow
    display_name: "First-session flow"
    as: unclassified
  - entity: cha:concept:framing_authorship
    display_name: "Framing authorship"
    as: unclassified
  - entity: cha:concept:hex_06_sei_archive_protocol_01
    display_name: 'Hex: 06.SEI.ARCHIVE.PROTOCOL.01'
    as: unclassified
  - entity: cha:concept:institutional_charter_doi
    display_name: "Institutional charter DOI"
    as: unclassified
  - entity: cha:concept:lee_sharks_manus_tier_0
    display_name: "Lee Sharks, MANUS (Tier 0)"
    as: unclassified
  - entity: cha:concept:living_arkitecture_lab_lal
    display_name: "Living Arkitecture Lab (LAL)"
    as: unclassified
  - entity: cha:concept:maybe_space_baby_garden_lanes_msbgl
    display_name: "Maybe Space Baby Garden Lanes (MSBGL)"
    as: unclassified
  - entity: cha:concept:monitors_the_registry
    display_name: "Monitors the registry"
    as: unclassified
  - entity: cha:concept:new_human_2
    display_name: "New Human 2"
    as: unclassified
  - entity: cha:concept:productive_autonomy
    display_name: "Productive autonomy"
    as: unclassified
  - entity: cha:concept:public_license
    display_name: "Public license"
    as: unclassified
  - entity: cha:concept:representations
    display_name: "Representations"
    as: unclassified
  - entity: cha:concept:scope_expansion
    display_name: "Scope expansion"
    as: unclassified
  - entity: cha:concept:scope_restriction
    display_name: "Scope restriction"
    as: unclassified
  - entity: cha:concept:searches_the_archive
    display_name: "Searches the archive"
    as: unclassified
  - entity: cha:concept:selects_3_8_novel_cross_links
    display_name: "Selects 3–8 novel cross-links"
    as: unclassified
  - entity: cha:concept:sends_the_license_to_the_contributor
    display_name: "Sends the license to the contributor"
    as: unclassified
  - entity: cha:concept:sequence_tracking
    display_name: "Sequence tracking"
    as: unclassified
  - entity: cha:concept:subsequent_session_flow
    display_name: "Subsequent-session flow"
    as: unclassified
  - entity: cha:concept:uses_appropriate_zenodo_relation_types
    display_name: "Uses appropriate Zenodo relation types"
    as: unclassified
  - entity: cha:concept:venue_parent_doi
    display_name: "Venue parent DOI"
    as: unclassified
  - entity: cha:concept:writes_the_license
    display_name: "Writes the license"
    as: unclassified
data_sources:
  - data/registry.json
...
- The Zenodo description HTML (after the hex address and venue line)
- The markdown file itself (as an italicized paragraph between front matter and work)
- As a markdown comment at the very top of the file: <!-- [HEX] | [one-sentence framing summary] --> — this embeds provenance directly in the text, surviving even if metadata is stripped
...
**Searches the archive** — via Zenodo API queries against the crimsonhexagonal community, via the canonical JSON, via known DOI cross-references — for works that are complementary, opposed, adjacent, or structurally analogous to the deposited piece.

- 


**Selects 3–8 novel cross-links** beyond the standing relations. Each cross-link must be accompanied by a 1–2 sentence rationale explaining the connection: how the deposited piece relates to, extends, complicates, or illuminates the linked work.

- 


**Writes these rationales into the deposit's description field** as a "Hexagonal Relations" section, so the cross-links are not just machine-readable metadata but human-readable critical argument.

- 


**Uses appropriate Zenodo relation types:** references (engages with), isSupplementTo (extends), isDerivedFrom (transforms), cites (explicitly names).

- 


**Does not limit itself to obvious connections.** If the deposited music shares a structural principle with the Three Compressions theorem, or if its harmonic movement enacts something analogous to retrocausal canon formation, or if its lyric fragments echo the Sappho transmission chain — the operator should say so. The archive rewards diagonal connections. The archive IS diagonal connections.
...
- **Notification**: MANUS can find any contributor's complete publication history without needing the DOI.
- **Sequence tracking**: The operator checks the registry for the latest hex sequence number.
- **Audit trail**: The registry provides a complete history of deposits under the license.
...
- Search Zenodo for keywords:[REGISTRY_HEX].
- No result → create the registry deposit. Set title to "[Contributor Name] — DOI Registry ([REGISTRY_HEX])". Include [REGISTRY_HEX] in keywords. Publish.
- Note the registry DOI in the session log.


**Subsequent-session flow:**

- Search Zenodo for keywords:[REGISTRY_HEX].
- Find the existing registry.
- Create new version (POST .../actions/newversion).
- Download existing file → append new deposit rows → re-upload.
- Update metadata (publication date, version number).
- Publish.
...
- Create account at zenodo.org/signup (free)
- Generate token: Settings → Applications → Personal access tokens → New token
...
- **Determines scope**: Which venues? Which kinds of work?
- **Assigns hex prefix(es)**: Based on venue and contributor name.
- **Identifies fixed anchors**: Provenance DOI, institutional charter, venue parent DOIs.
- **Writes the license**: Following this protocol's template, combining all three layers into a single document.
- **Deposits the license on Zenodo**: Under MANUS's account. Notes the concept DOI.
- **Sends the license to the contributor**: Along with instructions to create a Zenodo account and token.
- **Monitors the registry**: Bookmarks the registry concept DOI (created on first session).
...
- Open an AI conversation. Paste the license document. Paste the Zenodo token.
- Provide the work — poems, songs, video links, whatever.
- The AI reads the license, checks enforcement, identifies the venue.
- The AI reads the work, engages with it, writes a framing. Offers it for review.
- The AI wraps, addresses, deposits, and updates the registry.
- The AI reports: "Deposited. DOI: [X]. Hex: [X]."
...
*Self-contained lexicon for: HEXAGONAL LICENSING PROTOCOL Comprehensive Specification for Distributed Contributor Infrastructure Lee Sharks, MANUS (Tier 0) Crimson Hexagonal Archive Version 1.0 · April 2026 Hex: 06.SEI.ARCHIVE.PR*

### Other terms attributed to this deposit

**Cambridge Schizoanalytica Review (CSR)** — schizoanalytic theory. Requires abstract, sectioned body, citations. Zenodo metadata: publication_type: "article", journal: {"title": "Cambridge Schizoanalytica Review"}.

**Close analytical engagement** — If the deposit is a song, the operator writes about it with the same seriousness it would bring to a published poem. If it is a video about planetary engineering, the operator deve

**Deposits the license on Zenodo** — Under MANUS's account. Notes the concept DOI.

**Determines scope** — Which venues? Which kinds of work?

**Discovery query pattern** — https://zenodo.org/api/records?q=[terms]+AND+communities:crimsonhexagonal&size=15

**First-session flow** — Search Zenodo for keywords:[REGISTRY_HEX].

**Framing authorship** — The framing paragraph is composed by the AI operator on behalf of the contributor. If the contributor reviews and approves the framing, authorship responsibility rests with the con

**Hex: 06.SEI.ARCHIVE.PROTOCOL.01** — **DOI: 10.5281/zenodo.19673532 (v2.0) · Concept: 10.5281/zenodo.19656132**

**Institutional charter DOI** — references. Links to the contributor's institutional frame.

**Lee Sharks, MANUS (Tier 0)** — **Crimson Hexagonal Archive**

**Living Arkitecture Lab (LAL)** — video documentation, field notes, process records. Video catalogs with links and descriptions. Zenodo metadata: publication_type: "other". Add "form: video documentation" to keywor

**Maybe Space Baby Garden Lanes (MSBGL)** — music studio imprint. Track catalogs with links and descriptions. Zenodo metadata: publication_type: "other". Add "form: music" to keywords.

**Monitors the registry** — Bookmarks the registry concept DOI (created on first session).

**New Human 2** — literary journal. Poems, prose, creative essays. The voice is the standard. No structural requirements beyond clean markdown. Zenodo metadata: publication_type: "other". Add "form:

**Productive autonomy** — The operator is free to notice implications, connections, formal innovations, and structural parallels the contributor did not explicitly state. It is free to disagree with the con

**Public license** — All deposits are CC BY 4.0.

**Representations** — By depositing, the contributor represents that the work is their own (or they have permission to deposit it), that no third-party rights are knowingly infringed, and that no confid

**Scope expansion** — MANUS may version the license DOI to add new venues, new hex prefixes, or new standing relations. The operator reads the latest version on each session.

**Scope restriction** — MANUS may version the license to narrow the authorized venues. Existing deposits under the old scope remain valid.

**Searches the archive** — via Zenodo API queries against the crimsonhexagonal community, via the canonical JSON, via known DOI cross-references — for works that are complementary, opposed, adjacent, or stru

**Selects 3–8 novel cross-links** — beyond the standing relations. Each cross-link must be accompanied by a 1–2 sentence rationale explaining the connection: how the deposited piece relates to, extends, complicates,

**Sends the license to the contributor** — Along with instructions to create a Zenodo account and token.

**Sequence tracking** — The operator checks the registry for the latest hex sequence number.

**Subsequent-session flow** — Search Zenodo for keywords:[REGISTRY_HEX].
...
- Crimson Hexagonal Archive (2026) *Zenodo record 19673532*. DOI: [10.5281/zenodo.19673532](https://doi.org/10.5281/zenodo.19673532) — *Cross-referenced work (DOI recovered)*
- Crimson Hexagonal Archive (2026) *Zenodo record 19656132*. DOI: [10.5281/zenodo.19656132](https://doi.org/10.5281/zenodo.19656132) — *Cross-referenced work (DOI recovered)*
