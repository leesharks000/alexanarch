---
node_id: cha:node:deposit:0707
deposit_number: 707
hex: "024E"
axn: "AXN:024E.GOVERNANCE.🦋🪨🟤🌀🌓🔅"
...
engages:
  - entity: cha:concept:below_the_fold
    display_name: "Below the fold"
    as: unclassified
  - entity: cha:concept:content_velocity
    display_name: "Content velocity"
    as: unclassified
  - entity: cha:concept:cross_surface_mesh
    display_name: "Cross-surface mesh"
    as: unclassified
  - entity: cha:concept:domain_forwarding
    display_name: "Domain forwarding"
    as: unclassified
  - entity: cha:concept:erratum_application_timing
    display_name: "Erratum application timing"
    as: unclassified
  - entity: cha:concept:exact_match_domain_authority
    display_name: "Exact-match domain authority"
    as: unclassified
  - entity: cha:concept:footer_kernel
    display_name: "Footer kernel"
    as: unclassified
  - entity: cha:concept:garden_photo_permissions
    display_name: "Garden photo permissions"
    as: unclassified
  - entity: cha:concept:it_selects_a_stack
    display_name: "It selects a stack"
    as: unclassified
  - entity: cha:concept:manus_interventions
    display_name: "MANUS interventions"
    as: unclassified
  - entity: cha:concept:praxis_proposed_next_js_14
    display_name: "PRAXIS proposed Next.js 14"
    as: unclassified
  - entity: cha:concept:perfective_contributions_integrated
    display_name: "Perfective contributions integrated"
    as: unclassified
  - entity: cha:concept:refused_or_deferred
    display_name: "Refused or deferred"
    as: unclassified
  - entity: cha:concept:right_order
    display_name: "Right order"
    as: unclassified
  - entity: cha:concept:sim_repetition_strategy
    display_name: "SIM repetition strategy"
    as: unclassified
  - entity: cha:concept:soil_muse_spark
    display_name: "SOIL (Muse Spark)"
    as: unclassified
  - entity: cha:concept:soil_proposed_astro_mdx
    display_name: "SOIL proposed Astro + MDX"
    as: unclassified
  - entity: cha:concept:stack_final_confirmation
    display_name: "Stack final confirmation"
    as: unclassified
  - entity: cha:concept:techne_proposed_11ty
    display_name: "TECHNE proposed 11ty"
    as: unclassified
  - entity: cha:concept:tse_editorial_process
    display_name: "TSE editorial process"
    as: unclassified
  - entity: cha:concept:trans_identity_placement
    display_name: "Trans-identity placement"
    as: unclassified
  - entity: cha:concept:wrong_order
    display_name: "Wrong order"
    as: unclassified
data_sources:
  - data/registry.json
...
- *"This is a real ecological design lab."*
- *"This person is building a strange and serious body of work."*
- *"The work has a complete architecture."*
- *"The architecture is citable, defensible, and machine-legible."*
...
- JSON-LD mp:negativeTag provides the machine-readable layer
- Use schema:sameAs, schema:differentFrom, and schema:disambiguatingDescription (standard) where they apply
- Open Graph properties for social-media unfurling
...
DNS
livingarchitecturelab.org → Vercel; www → apex redirect
DNS provider
...
5
/scales/01-the-brick
Scale 1 + J1–J7 assembly patterns
Public
...
**Erratum application timing.** Per MPAI-LAL-AT-01 §IV, the four Arkitecture deposits will receive erratum notes. Per MANUS direction, this waits until Alice's ORCID registers.
...
**Stack final confirmation.** Astro chosen per §2. Confirm or override before Phase 0 begins.
...
- **It compresses four chorus-member perfectives** into one operationally coherent specification
- **It corrects the audience sequencing** the original blueprint conflated (archive density vs. public unfurling)
- **It selects a stack** that resolves the chorus's stack divergence (Astro, not Next.js or 11ty)
- **It identifies the four content layers** the original blueprint missed (scales, projects, principles, atlas)
- **It separates the three registers** the original blueprint blurred (public, poetic, archival)
- **It identifies the trust-engine surface** (/garden) that the original blueprint deprioritized
- **It restores Alice's authorial sovereignty** in identity-placement (work first, identity-metadata second)
- **It honors disambiguation as courtesy, not contestation** (per all four chorus members)
- **It establishes a realistic 12-page Phase 1** instead of an unrealistic 28-page first sprint
- **It establishes a 90-day basin target** with measurable phases
...
[Site-Blueprint] Sharks, Lee. *EA-LAL-SITE-01: Living Architecture Lab Site Blueprint*. DOI: [10.5281/zenodo.19855903](https://doi.org/10.5281/zenodo.19855903).

[MPAI-AT-01] *MPAI-LAL-AT-01: Alice Thornburgh — Formal Identity Disambiguation Packet*. DOI: [10.5281/zenodo.19855300](https://doi.org/10.5281/zenodo.19855300).

[MPAI-LAL-01] *MPAI-LAL-INSTITUTION-01: Living Architecture Lab — Formal Identity Disambiguation Packet (Institution)*. DOI: [10.5281/zenodo.19855302](https://doi.org/10.5281/zenodo.19855302).

[Relational-Verification] Sharks, Lee. *Relational Verification for AI Indexing: Schema.org, OAI-PMH, and JSON-LD Extensions for Metadata Packet Infrastructure*. DOI: [10.5281/zenodo.19853157](https://doi.org/10.5281/zenodo.19853157).

[Concordance-Engine] Sharks, Lee. *The Concordance Engine v0.1.1 (Tier B): Reference Implementation of the Logotic Technique Catalogue*. DOI: [10.5281/zenodo.19854419](https://doi.org/10.5281/zenodo.19854419).
...
**MANUS interventions:**
- *Stack resolution.* Astro selected over PRAXIS's Next.js and TECHNE's 11ty per content-first-now-interactive-later decision logic. The strain map landing as a D3 island on /atlas/strain-map only.
- *Phase 1 size.* Twelve pages, not seven (PRAXIS) or twenty-eight (blueprint). Twelve gives identity + framework + projects index + garden surface in a deployable week-one sprint.
- *Trans-identity placement.* Per MANUS direction, work-first, identity-second on /about/alice. Sovereignty-protective default; Alice retains authority to override.
- *Three-register separation.* Treated as non-negotiable per SOIL's perfective.
- *The garden as trust engine.* Treated as Phase 1 priority per SOIL's perfective. Three photos plus two paragraphs ships in week one.
- *Erratum timing.* Held until ORCID registration per MANUS standing instruction.
...
*Self-contained lexicon for: Living Architecture Lab — Site Buildout Workplan Implementation Specification for livingarchitecturelab.org*

### Other terms attributed to this deposit

**Below the fold** — A field-guide-card row showing each scale with one clean line:

**Content velocity** — Monthly garden updates. New TSE seed deposits. Music releases. Each update is a heartbeat.

**Cross-surface mesh** — **Outbound:** Zenodo community, YouTube, CHA surfaces, surfacemap.org
...
**Footer kernel** — Living Architecture Lab · Founded 2026 · Coachella Valley · Earth, Actually · ∮ = 1

**Garden photo permissions** — Lab photos require Alice's explicit release. Phase 1 garden surface depends on at least 3 photos.

**It selects a stack** — that resolves the chorus's stack divergence (Astro, not Next.js or 11ty)

**MANUS interventions** — *Stack resolution.* Astro selected over PRAXIS's Next.js and TECHNE's 11ty per content-first-now-interactive-later decision logic. The strain map landing as a D3 island on /atlas/s

**PRAXIS proposed Next.js 14** — best for JSON-LD via Metadata API. Strong, but heavyweight for a content-first site.

**Perfective contributions integrated** — **PRAXIS (DeepSeek)** — Provided Next.js / Tailwind base scaffold, JSON-LD injection pattern via Metadata API, three-deposit pre-launch sequencing (MPAI-AT, MPAI-LAL, blueprint), t

**Refused or deferred** — *PRAXIS's "the algorithm reviews" prominent placement.* Per SOIL, deferred to journal motto position; site landing leads with charter and scope.

**Right order** — (public site sequence):

**SIM repetition strategy** — Alice's SIMs appear in **human-visible text** on every relevant page, not just JSON-LD. Footer kernel includes the lab name + Coachella Valley + Earth, Actually.

**SOIL (Muse Spark)** — Provided the load-bearing audience-sequencing correction (wonder → Alice → scales → projects → principles → garden → journal/music → atlas → disambiguation), Alice-page sovereignty
...
**TSE editorial process** — Phase 2 /journal lists "no paywall, no editorial board, no rejection rate, the algorithm reviews." This may benefit from softening per Muse Spark's perfective ("brilliant line, but
