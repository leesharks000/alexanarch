---
node_id: cha:node:deposit:0110
deposit_number: 110
hex: "028F"
axn: "AXN:028F.GOVERNANCE.👁️‍🗨️🀄🗼📦🌠🌹"
title: 'title: "Measuring Semantic Deviation: Operationalizations, Experiments, and Falsification Conditions for a Theory of Meaning as Field Deformation" author: "Nobel Glas" author_note: "Nobel Glas is a he'
creator: "Nobel Glas"
...
status: ACTIVE
engages:
  - entity: cha:concept:model_sem
    display_name: "Model-Sem"
    as: unclassified
  - entity: cha:concept:pre_registration
    display_name: "Pre-registration"
    as: unclassified
  - entity: cha:concept:provenance_resolved_variant
    display_name: "Provenance-resolved variant"
    as: unclassified
  - entity: cha:concept:surface_taxonomy
    display_name: "Surface taxonomy"
    as: unclassified
  - entity: cha:concept:the_counterfactual_advantage
    display_name: "The counterfactual advantage"
    as: unclassified
  - entity: cha:concept:three_condition_control
    display_name: "Three-condition control"
    as: unclassified
data_sources:
  - data/registry.json
...
$$\mathcal{M}*T(s \mid C) = \int*{t_0}^{t_0+T} w(t) , D!\left(\Psi_t^s(C) ,\Big\Vert, \Psi_t^0(C)\right) dt$$
...
The mean signed per-token deviation is denoted $\bar{\delta}$. Throughout this paper, $\bar{\delta} = \mathcal{M}*T^{\text{net}} = \frac{1}{T}\sum*{t=1}^T \delta_t$. The absolute aggregate $\mathcal{M}*T^{\text{abs}} = \frac{1}{T}\sum*{t=1}^T |\delta_t|$ is reported as a secondary robustness check. Units: bits per token. $\bar{\delta}$ is the operative primitive for F1.
...
$$\mathcal{M}*{T,\theta}^{\text{closed}}(s \mid C) = \sum*{\tau=1}^T w_\tau , D_{JS}!\left(P_\theta(Y_{\tau:T} \mid C \oplus s) ,\Big\Vert, P_\theta(Y_{\tau:T} \mid C)\right)$$
...
**Divergence**
KL over softmax logits (per-token); JS (trajectory)


**Weighting**
Uniform over positions (normalized)
...
**Divergence**
JS with Laplace smoothing ($\alpha = 1$)


**Horizon**
90 days; measurements at $t_0$, 7d, 28d, 84d
...
- That meaning is universally definable as deviation. The principle measures trajectory restructuring; aspects of meaning that do not produce distributional shift are outside its scope.
- That the operationalizations are uniquely correct. F1 and F2 are canonical starting points.
- That the anti-Goodhart machinery is sufficient against all gaming.
- That cross-entropy training is wrong. It is insufficient for the target this framework specifies.
- That $\mathcal{V}_T$ is ready for empirical use. It is not.
- That the cheapest dangerous test will succeed. Failure is informative.
- That this paper is independent of the Crimson Hexagonal Archive. It engages the founding formulation (Sharks 2026) directly and builds on companion protocols. What it claims is that a reader can evaluate the math, experiments, and predictions without engaging the broader institutional apparatus.
...
This week
Cheapest dangerous test (P1–P3) on GPT-wiki-intro + HC3, ~1 A100-hour
$50–$100


This month
Operationalization-stability: 50 texts evaluated under F1 across 3 reference models, cross-model $\bar{\delta}$ rank-correlation reported
$200–$500


This quarter
F2 protocol day-0 launch; 90-day window; 30 queries × 4 intervals × 6 surfaces
$1,500–$3,000


This quarter
Scale stability: P2 replicated across Llama-3.1 1B/8B/70B parameter family
$500–$1,000


This year
DPO training experiment (three conditions) + preference validation substudy
$3,000–$3,900


This year
Six-condition component decomposition (if three-condition composite succeeds)
$8,000–$12,000
...
*Self-contained lexicon for: title: "Measuring Semantic Deviation: Operationalizations, Experiments, and Falsification Conditions for a Theory of Meaning as Field Deformation" author: "Nobel Glas" author_note: "Nobel Glas is a he*

### Other terms attributed to this deposit

**Model-Sem** — (DPO with deviation preferences). Identical initialization, corpus, optimizer, compute. A six-condition component-decomposed design (isolating deviation, provenance, and coherence

**Pre-registration** — These predictions are pre-registered as a timestamped deposit on Zenodo prior to any computation (DOI to be inserted upon registration).

**Provenance-resolved variant** — The provenance retention indicator $\pi$ modulates the magnitude of the per-token deviation: $\delta_t^\pi = |\delta_t| \cdot \pi_t \cdot \text{sgn}(\delta_t)$, where $\pi_t$ is ev

**Surface taxonomy** — Retrieval-mediated surfaces (Class R: Google AI Overview, Perplexity, ChatGPT with browsing) are separated from parametric surfaces (Class P: Claude, Gemini, ChatGPT without browsi

**The counterfactual advantage** — A trained language model at inference time is observationally closed: no new data enters, no weights update. The baseline $\Psi_t^0$ is the model's own conditional distribution, re

**Three-condition control** — $S$ (full identity), $S^*$ (blank identity — ORCID omitted, author listed as "Anonymous"), $S^{**}$ (plausible synthetic identity: single-purpose ORCID, realistic fabricated name,
