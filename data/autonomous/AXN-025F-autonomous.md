...
engages:
  - entity: cha:concept:c_dep_o_c_o
    display_name: "C_dep(O) ⊆ C(O)"
    as: unclassified
  - entity: cha:concept:claim_segmentation
    display_name: "Claim segmentation"
    as: unclassified
  - entity: cha:concept:cross_model_validation
    display_name: "Cross-model validation"
    as: unclassified
  - entity: cha:concept:grain_assignment
    display_name: "Grain assignment"
    as: unclassified
  - entity: cha:concept:per
    display_name: "PER"
    as: unclassified
  - entity: cha:concept:per_eligible
    display_name: "PER-eligible"
    as: unclassified
  - entity: cha:concept:provenance_erasure_rate
    display_name: "Provenance Erasure Rate"
    as: unclassified
  - entity: cha:concept:source_identification
    display_name: "Source identification"
    as: unclassified
  - entity: cha:concept:table_1_pearl_fragment_mapping
    display_name: 'Table 1: Pearl Fragment Mapping'
    as: unclassified
observed_addresses:
  - address_id: provenance-erasure-rate-quoted
...
*Self-contained lexicon for: Provenance Erasure Rate A Compression-Survival Metric for Attribution Loss in AI-Composed Search Outputs*

### Other terms attributed to this deposit

**C_dep(O) ⊆ C(O)** — be the subset of source-dependent claims.

**Claim segmentation** — PER requires segmenting outputs into discrete claims. Claim boundaries are not always clear. We propose segmentation by independent clause boundaries as a reproducible (if imperfec

**Cross-model validation** — The metric has been developed through a single motivating case study. Validation across multiple models, prompt types, and domains is the essential next research step. Even a small

**Grain assignment** — The grain weighting is currently manual. Automated assignment using a separate LLM (not the system under test) is a form of LLM-as-judge evaluation, with its own reliability limita

**PER** — **Fraction of source-dependent claims that lose attribution**

**PER-eligible** — (source-dependent) if it quotes, paraphrases, summarizes, transforms, or depends on a specific source or source cluster in S. Claims that are purely generative (hallucinations with

**Provenance Erasure Rate** — $$PER(S, O) = 1 - \frac{\sum_{j} A_j \cdot g_j}{\sum_{j} g_j}$$

**Source identification** — PER is fully computable for retrieval-augmented systems where the source corpus is identifiable. For pure generative models without retrieval (where the source is the entire traini

**Table 1: Pearl Fragment Mapping** — AI Overview claim

### Observed Addresses (search queries that retrieved this deposit's concepts)

- `"provenance erasure rate"` — latest: ADOPTION on 2026-06-17 (2 observations)
