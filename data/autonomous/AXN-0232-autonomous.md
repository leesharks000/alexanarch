...
engages:
  - entity: cha:concept:begin_personal_forensic_capture_immediately
    display_name: "Begin personal forensic capture immediately"
    as: unclassified
  - entity: cha:concept:chrome_web_store
    display_name: "Chrome Web Store"
    as: unclassified
  - entity: cha:concept:detection_strategy_layered
    display_name: "Detection strategy (layered)"
    as: unclassified
  - entity: cha:concept:direct_url_match
    display_name: "Direct URL match"
    as: unclassified
  - entity: cha:concept:direct_zenodo_deposit
    display_name: "Direct Zenodo deposit"
    as: unclassified
  - entity: cha:concept:domain_optional
    display_name: "Domain (optional)"
    as: unclassified
  - entity: cha:concept:domain_match
    display_name: "Domain match"
    as: unclassified
  - entity: cha:concept:draft_chrome_web_store_privacy_policy
    display_name: "Draft Chrome Web Store privacy policy"
    as: unclassified
  - entity: cha:concept:endpoint_specification
    display_name: "Endpoint specification"
    as: unclassified
  - entity: cha:concept:first_paper
    display_name: "First paper"
    as: unclassified
  - entity: cha:concept:forensic_case_studies
    display_name: "Forensic case studies"
    as: unclassified
  - entity: cha:concept:heuristic_detection
    display_name: "Heuristic detection"
    as: unclassified
  - entity: cha:concept:mutationobserver
    display_name: "MutationObserver"
    as: unclassified
  - entity: cha:concept:name_match
    display_name: "Name match"
    as: unclassified
  - entity: cha:concept:ongoing_series
    display_name: "Ongoing series"
    as: unclassified
  - entity: cha:concept:phrase_match
    display_name: "Phrase match"
    as: unclassified
  - entity: cha:concept:registered_works_manager
    display_name: "Registered Works Manager"
    as: unclassified
  - entity: cha:concept:reserve_domain
    display_name: "Reserve domain"
    as: unclassified
  - entity: cha:concept:rotate_github_pat_and_zenodo_token
    display_name: "Rotate GitHub PAT and Zenodo token"
    as: unclassified
  - entity: cha:concept:sourced_unattributed
    display_name: "SOURCED_UNATTRIBUTED"
    as: unclassified
  - entity: cha:concept:selector_based_detection
    display_name: "Selector-based detection"
    as: unclassified
  - entity: cha:concept:tagline_options
    display_name: "Tagline options"
    as: unclassified
  - entity: cha:concept:urls
    display_name: "URLs"
    as: unclassified
data_sources:
  - data/registry.json
...
**The user's browsing data belongs to the user.** The extension never accesses, logs, or transmits any data about what the user searches, visits, or does online — except for the specific AI Overview payloads the user explicitly chooses to contribute.
...
- **Direct URL match:** Is any source URL in the overview a registered work?
- **Domain match:** Does any source URL share a domain with a registered work?
- **DOI match:** Does any source resolve to a registered DOI?
- **Name match:** Does the overview text or any source title contain a registered author name?
- **Phrase match:** Does the overview text contain key phrases from the user's registered works without attribution?
...
- Their AI Overview is a transformative work that does not require attribution to its sources (justifying the erasure of creator names)
- Their AI Overview is proprietary content that cannot be quoted, displayed, or analyzed by those same creators
...
- **Build Phase 0 proof of concept** — content script that detects and parses AI Overview
- **Test against current Google SRP DOM structure** — validate detection selectors
- **Begin personal forensic capture immediately** — even a bare-bones extension that logs overviews to local storage is better than scanning by spidey sense
- **Reserve domain** if desired (overviewwatch.org / overviewwatch.dev)
- **Create Supabase table** for corpus (using existing Supabase connection)
- **Draft Chrome Web Store privacy policy**
- **Rotate GitHub PAT and Zenodo token** (still outstanding from April 6 session)
...
*Self-contained lexicon for: OVERVIEW WATCH: Comprehensive Development Plan Document ID: EA-OVW-PLAN-01*
...
**Detection strategy (layered)** — **Selector-based detection.** Google currently renders AI Overviews in identifiable container elements. These selectors change, but typically involve data attributes or specific cl
...
**Draft Chrome Web Store privacy policy** — **Rotate GitHub PAT and Zenodo token** (still outstanding from April 6 session)

**Endpoint specification** — POST https://[supabase-project].supabase.co/rest/v1/overview_corpus

**First paper** — "Attribution Rates in Google AI Overviews: Evidence from the Semantic Economy Attribution Corpus" — publishable once corpus reaches ~1000 contributed overviews

**Forensic case studies** — Individual PVE documents for notable instances of systematic erasure (building on PVE-003 methodology)

**Heuristic detection** — If selectors fail, fall back to heuristic: scan for content blocks that appear above organic results, contain synthesized prose (not snippets), and include inline source citations.
...
**Registered Works Manager** — Add/edit/remove DOIs, URLs, domains, names, phrases
...
**Selector-based detection** — Google currently renders AI Overviews in identifiable container elements. These selectors change, but typically involve data attributes or specific class patterns. The extension ma

**Tagline options** — *Who said it first?*
