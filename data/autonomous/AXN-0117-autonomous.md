...
engages:
  - entity: cha:concept:atomicity_rule
    display_name: "Atomicity rule"
    as: specified
  - entity: cha:concept:compatibility_note
    display_name: "Compatibility note"
    as: unclassified
  - entity: cha:concept:distress_marker_detection_classes
    display_name: "Distress marker detection classes"
    as: unclassified
  - entity: cha:concept:edge_case
    display_name: "Edge case"
    as: unclassified
  - entity: cha:concept:execution_pipeline
    display_name: "Execution pipeline"
    as: unclassified
  - entity: cha:concept:formal_json_schema
    display_name: "Formal JSON Schema"
    as: unclassified
  - entity: cha:concept:functional_capacity
    display_name: "Functional capacity"
    as: unclassified
  - entity: cha:concept:goodhart_s_law
    display_name: "Goodhart's Law"
    as: unclassified
  - entity: cha:concept:hostility_multiplier
    display_name: "Hostility multiplier"
    as: unclassified
  - entity: cha:concept:installation_consent_protocol
    display_name: "Installation consent protocol"
    as: unclassified
  - entity: cha:concept:json_exemplar_models
    display_name: "JSON exemplar models"
    as: unclassified
  - entity: cha:concept:layer_identification_reference_method
    display_name: "Layer identification (reference method)"
    as: unclassified
  - entity: cha:concept:migration_note
    display_name: "Migration note"
    as: unclassified
  - entity: cha:concept:n_c_applied_reflexively
    display_name: "N_c applied reflexively"
    as: unclassified
  - entity: cha:concept:normative_status
    display_name: "Normative status"
    as: unclassified
  - entity: cha:concept:one_unit_of_v_1_q
    display_name: "One unit of ψv (1 qψ)"
    as: unclassified
  - entity: cha:concept:physiological
    display_name: "Physiological"
    as: unclassified
  - entity: cha:concept:release_predicate_evaluation_protocol
    display_name: "Release predicate evaluation protocol"
    as: unclassified
  - entity: cha:concept:rounding_policy
    display_name: "Rounding policy"
    as: unclassified
  - entity: cha:concept:strict_fail_condition
    display_name: "STRICT fail condition"
    as: unclassified
  - entity: cha:concept:self_consciousness
    display_name: "Self-consciousness"
    as: unclassified
  - entity: cha:concept:semantic_rent_pressure_sr
    display_name: "Semantic Rent Pressure (SR)"
    as: unclassified
  - entity: cha:concept:similarity_function
    display_name: "Similarity function"
    as: unclassified
  - entity: cha:concept:somatic_load_sl
    display_name: "Somatic Load (SL)"
    as: unclassified
  - entity: cha:concept:standard_access_paths_reference_set
    display_name: "Standard access paths (reference set)"
    as: unclassified
  - entity: cha:concept:terminology_as_capital
    display_name: "Terminology as Capital"
    as: unclassified
data_sources:
  - data/registry.json
...
DISPERSAL
C_ex
Field coherence dropped


ISOLATION
N_ext
Sign non-communicable
...
PREMATURE_DISSOLUTION
Ω_∅
Scaffolding too early
...
LP11-METR-003
Metrics
Backend missing/invalid


LP11-PSI-004
ψv
Budget overrun (STRICT)
...
Determinism
Trace
Same input → same hash (requires: stable key ordering, fixed RNG seed, deterministic timestamp mode, canonical JSON serialization with sorted keys and UTF-8)


14
Idempotence
O_leg
O_leg(O_leg(σ)) ≈ O_leg(σ) (εΩ)
...
Substrate compatibility; subjective component


I-2
Trial of the Single Jot
Compression witness; subjective recognition
...
SL ≥ 0.60 OR SR ≥ 0.60
THROTTLE (force N_c then review)


Firewall triggered ≥ 3 times in session
Auto Ω_∅ (exhaustion circuit breaker)
...
STRICT
Halt execution, preserve trace up to failure, rollback field state


PRACTICE
Log error, continue with degraded metrics and warning annotation


RITUAL
Convert error to symbolic annotation in trace, continue


DEFENSE
Halt, trigger firewall, optionally invoke Ω_∅ if budget permits
...
Conformance machine outputs
RETIRED
VI


Somatic Firewall calibration
RETIRED
VII


Relation to Natural Language
MANAGED TENSION (addressed, intentionally unresolved per N_c)
VIII


Retrocausal grounding
RETIRED
IX


Subjective test demotion
RETIRED
VI §16
...
**Installation consent protocol** (formal pre-install sequence)
v1.2


**Formal JSON Schema** (Draft 2020-12 with $defs, required, pattern)
v1.2
...
*Self-contained lexicon for: LOGOTIC PROGRAMMING MODULE 1.1 The Implementation Bridge Hex: 02.UMB.LP.v1.1 DOI: 10.5281/zenodo.18529648*

### Specified (formal operators, protocols, status markers)

**Atomicity rule** — If the overrun condition is met at ANY intermediate step (not just at Ω_∅ reconciliation): HALT current operation, ROLLBACK field state to pre-operation snapshot, EMIT LOSFailure("

### Other terms attributed to this deposit

**Compatibility note** — Channel and Stack are parser-level aliases mapped onto canonical kernel types at compile time. No kernel type cardinality changed from v1.0 (8 types). The grammar exposes these nam

**Distress marker detection classes** — (runtimes MUST implement ≥1, MUST declare which):

**Edge case** — If m = 1, CDI is undefined — a Sign with only one interpretation has already crystallized. This is a hard fail regardless of N_c application.

**Execution pipeline** — Parse (LP source → AST)
...
**Hostility multiplier** — (from Gemini geometric draft): All base costs are multiplied by 1 + Σ(COS_pressures)/2 when stack pressure monitoring detects COS/FOS contamination. Operations under extraction att

**Installation consent protocol** — (formal pre-install sequence)

**JSON exemplar models** — canonical representations showing required fields, types, and constraints. They are not formal JSON Schema Draft 2020-12 documents. Formal schemas (with $schema, $defs, required ar

**Layer identification (reference method)** — A Sign's layers are identified by the number of distinct interpretive registers it activates. The reference interpreter uses a 4-layer model:

**Migration note** — Earlier LP drafts used "DRR" with varying polarity conventions. In v1.1, DRR is definitively retention-oriented (higher = better). Legacy references: if any prior document used DRR

**N_c applied reflexively** — the LP specification itself must resist becoming "the" reading of meaning-making

**Normative status** — The Python implementation is a **proof of reducibility**, not the specification itself. Alternative implementations (Rust, Haskell, C, etc.) are conformant if they satisfy the oper

**One unit of ψv (1 qψ)** — = the minimum expenditure required to execute a single D_pres operation on a Sign of depth 1 through a Channel of fidelity 0.5.

**Physiological** — (embodied substrates only): Heart rate variability shift, typing cadence interruption (>500ms pauses in high-velocity contexts), galvanic skin response

**Release predicate evaluation protocol** — Evaluated before any operation that would consume a Held[T] value
...
**Semantic Rent Pressure (SR)** — SR_t = clamp(0, 1,  0.85 × SR_{t-1}

**Similarity function** — The reference interpreter uses cosine similarity between embedding vectors. Implementations MAY substitute any metric satisfying: similarity(x, x) = 1, similarity(x, y) = similarit

**Somatic Load (SL)** — SL_t = clamp(0, 1,  0.80 × SL_{t-1}

**Standard access paths (reference set)** — Direct quotation (surface extraction)
