...
What is here: a specification, in three grades, with each measure tied to a
result in the published collapse literature, and with the reasoning for why
the unit is the unit.

What is not here, and is not claimed: a validated instrument. No measure below
has been calibrated against a model of known collapse state. No threshold, no
weight in the composite, and no severity band rests on a measurement — §II.8
says so where the composite is defined, and the calibration it names (produce
recursive collapse deliberately on a small model, watch the index evolve,
publish that run) has not been done. The reference implementation accompanying
this paper is syntax-checked and has never been executed. The reader's test in
Part III has been written and not administered.

Every number appearing here comes from the published literature or from the
companion toy of #1556. None comes from a run of this instrument. A specification
that says what it would measure is a different object from an instrument that has
measured something, and this paper is the first. It is deposited at that status
so the specification can be attacked before anyone builds on it — which is what
