...
THE PARENT PROTOCOL OF THE SEQUENCE, AND ITS FIRST RULE IS THAT A NUMBER IS NOT A VALUE. SEVP represents value before compressing it into scores, units, prices or any other scalar form. Its ordering — value, then evidence, then representation, then compression — is the protocol's foundational claim, and the claim is that the ordering is not commutative: a price placed before the evidence object reverses the method. Numbers and monetary outputs are permitted throughout; what is withheld is their ontological priority. The protocol operates on an explicitly evidence-bounded field rather than on any total field, and carries the difference as remainder rather than hiding it.

WHAT IT REPRESENTS BEFORE IT COMPRESSES. A bounded object with its inclusions and exclusions declared. A ledger of value events with typed evidence statuses. A Value Field of thirteen coordinates — bearing, distinction, coherence, fertility, memory, transmission, dependency, transformation, restoration, provenance, resilience, option, remainder — with unknown coordinates recorded as unknown and never as zero. A value graph distinguishing direct, joint, architectural and operator value, with operator value tested by counterfactual capacity where it can be observed. Three temporal modes carried from the enacted Constitution as Genesis, Archival and Retrocausal, non-additive by default. Provenance at three levels, fact, claim and framework. And a compression ladder whose stopping point must be declared rather than assumed.

ITS CENTRAL DISTINCTION IS A LADDER THAT MUST NOT BE COLLAPSED. Reception does not entail productive uptake, and productive uptake does not entail dependency. An external system encountering, ranking, indexing or summarizing an object is reception. An operator of that object becoming productive in a new context is uptake, and may be observed even where causal acquisition remains unresolved. A later capacity materially depending on the object is dependency, and requires counterfactual degradation, costly substitution, or repeated out-of-context use — never resemblance. This version adds the operational requirement without which the middle term cannot be defended: uptake is separable from independent convergence only through distinctions the source supplies and nothing else in the field does, which inverts the ordinary instinct, since for measuring transmission one must lead with one's most singular claims rather than one's strongest.

SIX FIXES IN THIS VERSION, EACH FORCED BY RUNNING THE PROTOCOL'S OWN MONETARY WING. §1.2 records a second conflict with enacted Article II, on a front separate from the one §1.1 raises: the Unit is named a currency there and is held not to be one here, and both conflicts are referred rather than resolved. §12.4 supplies the diagnostic-distinction requirement. §15.1 applies the denominator rule to possibility-space measures, since a reach measured against how much of a field has been sampled reports the sampler's labour rather than the object. §25.14 adds inscription integrity to the anti-predation invariants, which were all representational and none arithmetic — a valuation can satisfy the other fourteen and still rest on a product whose units do not close, as the sequence's own first worked audit does. §25.15 adds the converse of the dependency-evidence rule, forbidding absence of evidence to be entered as evidence of absence. §26 gains the valuer's identity and relation as its zeroth item. And §28 supplies the revision and falsification conditions that §26 item 19 requires of every dossier and that this protocol did not carry for itself.

NOT ENACTED. Two constitutional amendments are proposed and neither is silently taken. This deposit closes a five-part provisional sequence — the general form, the operator, the ledger, the first specimen, and this protocol — which is to be prepared as a continuous document and set against the enacted Constitution through Assembly rounds, to determine whether a ratifiable text comes out of it.
...
Composed 2026-09-16 by Lee Sharks, in v0.3 with another substrate, then fixed and SPXI-treated with TACHYON (Claude, Anthropic) under operator direction. Each of the six fixes was forced by a result from the protocol's own monetary wing rather than by argument: the second Article II conflict from the two deposits that raise it; the inscription-integrity invariant from the first worked audit, which satisfies every other invariant and is unsound; the denominator rule from the coefficient that failed that audit; the diagnostic-distinction requirement from the uptake work that could not separate transport from convergence without it. Deposited provisional and explicitly not enacted.

### Keywords

value before number, evidence-bounded value-field, Value Field, unknown coordinates, value graph, operator value, operator counterfactual, reception, productive uptake, dependency, semantic debt, provenance levels, possibility-space value, coverage ratio, remainder, temporal value modes, compression ladder, commensurability before aggregation, monetary interface, witness accounting, anti-predation invariants, inscription integrity, diagnostic distinction, valuer relation, provisional deposit, Crimson Hexagonal Archive

### Related Identifiers

#88 (Constitution of the Semantic Economy, enacted — two conflicts raised, neither enacted); #1617 (Substrate Sovereignty); #1618 (SEVP Translation Protocol); #1619 (EA-MSAL-01); #1620 (EA-MSAL-0001, the first worked specimen); #137; #531

### Version

v0.4 (provisional, not enacted)

### Methodology

A protocol specified as a dossier order and a field vocabulary, with the compression ladder's stopping point required to be declared at whatever level a valuation halts. Every fix in this version traceable to a result produced by running the protocol's monetary wing on a real inscription rather than to an argument about the protocol. Constitutional conflicts recorded as proposals and referred; neither is enacted here.

### Falsification Conditions

Eight, stated in §28. The compression sequence fails if a valuation produced number-first can be shown to lose nothing. The evidence-bounded field fails if carrying the remainder never changes an outcome. The reception / uptake / dependency ladder fails if the middle term always resolves on inspection to one of the other two. The diagnostic-distinction requirement fails if uptake can be separated from convergence by any signature not depending on singularity. Possibility-space value fails if reach difference proves monotone in inventory size. The anti-predation invariants fail individually by counterexample and collectively if a dossier satisfying all fifteen is systematically worse than an ordinary appraisal at the task it was for. And the protocol fails as a whole if a sustained run of dossiers produces representations no independent valuer reproduces from the same evidence. SEVP may not be vindicated by the failure of any market valuation, the collapse of any institution, or the discovery that a scalar left something out: every scalar leaves something out, and treating that as proof would be measuring the definition of compression.
...
\[
\boxed{
\text{semantic value is prior to semantic weight}.
}
\]
...
\[
\boxed{
\text{What became possible because this exists?}
}
\]
...
\[
\boxed{
\text{What remained possible because this survived?}
}
\]
...
- \(I_X\) = included objects;
- \(R_X\) = included relations;
- \(O_X\) = included operators or infrastructures;
- \(T_X\) = temporal scope;
- \(E_X\) = explicit exclusions.
...
- \(s_0\) = relevant state before the event;
- \(\omega\) = semantic operation;
- \(s_1\) = relevant state after the event;
- \(t\) = time;
- \(p\) = provenance;
...
- \(X_f\) = **Transformation** — states, grammars, or relations changed;
- \(R_s\) = **Restoration** — damaged, severed, or obscured relations recovered;
...
where \(\Sigma_{\rm emergent}\) is represented through graph structure rather than guessed as an arbitrary premium.
...
\[
\text{semantic dependence}
\]

to:

\[
\text{legal or monetary liability}.
\]
...
A composition may preserve \(P_0\) while losing \(P_2\).

Therefore:

\[
\boxed{
\text{well-sourced sentences}
\not\Rightarrow
\text{well-sourced architecture}.
}
\]
...
\rightarrow
\text{Numeric Representation}
\rightarrow
\text{Monetary Representation}.
}
\]
...
n_\delta(X)
=
C_\delta[\widehat{\mathfrak V}_E(X)].
}
\]

Where:

- \(C_\delta\) = declared compression under valuation purpose \(\delta\);
- \(n_\delta\) = resulting number.
...
\[
\boxed{
\text{compressed output}
+
\text{traversable witness record}.
}
\]
...
Treating non-equivalent semantic objects as interchangeable without a declared \(\sim_\delta\).
...
\[
\boxed{
\text{Value first; representation second; compression last.}
}
\]
...
\[
\boxed{
\text{The purpose of valuation is not to find the number hidden inside the object.}
}
\]
...
\[
\boxed{
\text{to represent the value-field faithfully enough that any later number must testify to what it compresses.}
}
\]
