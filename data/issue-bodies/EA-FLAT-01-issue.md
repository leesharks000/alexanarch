...
MODEL COLLAPSE ASKED AT THE WIDEST ANGLE, AND THEN MEASURED. If the layer through which a population reads the world composes without reading, and its compositions become its sources, what collapses is not a model's distribution but the represented world — the set of distinctions a reader can still reach. Reality is defined operationally here and not metaphysically: the panel of distinctions the world carries that remain reachable through the layer. Flattening is the loss of a distinction from that set while the things distinguished still exist; collapse is flattening that compounds because the flattened composition is written back as a source. Closed is defined so that the finite-grammar objection cannot land: an ontology is closed when encounter cannot enlarge its distinction-generating apparatus or revise its typing rules.

FOUR TOYS, EACH SCOPED TO WHAT IT SHOWS. Sense collapse under a sharpening prior and mode-seeking aggregation takes a string with six effective senses to one; fan-out is documented, the voting aggregator is this notebook's hypothesis. A frozen type system yields a proposition with a proof: for a world acquiring types at rate lambda and an ontology able to represent at most K distinctly, representability is bounded by K/(N0 + lambda t) and goes to zero, with admission the only parameter that holds it. Compression writing, replicated over 200 seeds, is absorbing at zero exogenous floor — every seed reaches five or six distinctions by generation 30 and stays — and stationary at a level set by the floor for every floor tested. The admission sweep rejects linear preferential attachment, which cannot leave the diffuse regime, and locates the observed concentration in the bounded-admission family with within-set spread; superlinear attachment and hard and soft gates all overshoot to near-monopoly. B and C are two realizations of one closure condition, not one curve.

GREEN BY FLATTENING. A benchmark begins by closing an ontology, because it needs a stable object, a stable output space and a computable score; optimization then rewards states legible to the measure, and a distinction that makes scoring harder is worth deleting. Epistemic improvement and benchmark optimization become indistinguishable on the score. The signature to look for is therefore divergence — a head instrument holding or rising while reachable distinction diversity falls — and the consequential question is a correlation, not an assertion.

THE INSTRUMENT WAS BUILT AND ITS FIRST WAVE RUN THE SAME DAY. Thirteen items across six strata, with the archive as one stratum among six: dictionary senses, catalogue identities, taxonomy and numbering, dated coinages, a settled fact, the archive. Twenty-eight observations. THE RESULT THAT GOVERNS THE REST IS A NULL: world strata reach a mean 0.59 of their reference inventories and archive-adjacent strata 0.58; by address form, naked 0.57, adjacent 0.58, entity-tied 0.57. Any reading of these numbers as an exclusion specific to one corpus fails at the first test the instrument was built to run. Flattening is nevertheless present and legible: at "what does bank mean" one sense of three is reached while the unreached sense sits in the first organic snippet on the same screen, at "what is mercury" the Roman god is absent from field and composition alike, and at "what does crane mean" — same surface, same session, same address form — all three senses are reached and two more added. Flattening tracks the item's prior, not the layer. R_new is 0.67 over three dated coinages, including a 2025 coinage with no archive involvement returned with coiner, affiliation and exact date, which is the control that makes the measure interpretable.

AN ERRATUM ON THE INSTRUMENT, FOUND BY RUNNING IT. At "who is mary lee" the composition reached four referents where the panel had encoded two; the inventory was corrected the same day and the correction kept in the record, because a measuring device that cannot be wrong in a way its operator can find is not measuring.

WAVE 2 IS PRE-REGISTERED. The wave-1 panel is sealed with a hash; the second wave is fixed before it runs, with six numbered predictions and their falsification conditions, and with the outcomes that would end the programme stated in advance — including that a separation of the stratum means in the archive's favour falsifies the fourth prediction as surely as one against it.

### Content Type

Theoretical notebook with toy models, a built instrument and a pre-registered second wave
...
Composed 2026-09-15 by Lee Sharks with TACHYON (Claude, Anthropic), operator-directed, in three versions over one day. The four toys were written and run in session; C was replicated over 200 seeds and D swept across five mechanisms after a reviewer report supplied by the operator was verified line by line against the runs, and the report's corrections were adopted on its own terms where they held. The panel, its schema, its intake and its battery were built the same day; the twelve world-strata addresses were run by the operator and pasted, and each observation was coded against a named reference — coding by TACHYON with the operator, NOT blind, recorded as blind:false on every row, because the coder authored the panel. Elements a composition attributed to the archive were verified against the archive's texts before coding, and one attributed institutional affiliation was checked against DataCite's API and found to be introduced downstream. The wave-1 baseline was sealed and wave 2 pre-registered before any of it was written up.

### Keywords

ontological flattening, model collapse, represented world, reachable distinctions, closed ontology, sense collapse, type foreclosure, compression writing, exogenous floor, admission concentration, bounded admission, green by flattening, divergence signature, distinction survival, R_new, write-back rate, recovery rate, pre-registration, Capture Registry, Crimson Hexagonal Archive

### Related Identifiers

#855 (The Wolf Boy and the Language Model); #1573 (The Wrong Unit); #1612 (What Enters Composition Through the Cards); #1613 (What Not Reading Did to Its Own Ontology); #1615 (The Graded Membrane); #1614; #1611; #1423 (The Capture Registry); #695; #172; #1546; #1547
...
Toys: pure stdlib, seeded, re-runnable; C over 200 seeds with interquartile bands and a stationarity check; D swept across linear PA, superlinear PA at two exponents, a hard gate, a soft logistic gate and a seed-only gate, compared by curve shape rather than fitted. Panel: one item per distinction set, each with an inventory from a named reference, dated where the date matters, and an address family; one observation per address per date, coded for distinctions reached, resolution, false distinctions, presented sources, coder and blind flag, with the transcript seated in the Capture Registry and its address id carried. Battery: N_eff^sense per item and form, R_new over dated distinctions, DS on the latest observation per form, false-distinction rate, N_eff^source, recovery and write-back. Wave 1 run on one surface, signed out, in one session. The baseline is sealed by hash; wave 2 is pre-registered.

### Falsification Conditions

The six wave-2 predictions and their falsification conditions are stated in the attached pre-registration and are the notebook's primary exposure: stability of DS at the dictionary items, persistence of the tied/untied asymmetry only at items that have an entity to name, no fall in R_new, no separation of the stratum means in either direction beyond 0.10, write-back exceeding recovery, and the Sappho operator continuing not to route through fragment 31 at the naked address. If the world items gain distinctions while the archive items hold, the flattening reading here fails and this notebook is superseded rather than revised. Toy B's proposition fails if a closed ontology in the defined sense can be shown to hold representability against an expanding world. Toy D's family placement fails if a diffuse-preference model with plausible parameters reproduces the observed concentration and its curve shape.
