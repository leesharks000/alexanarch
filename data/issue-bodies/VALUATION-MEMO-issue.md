...
EA-VALUATION-MEMO-01: The Machine Valuation of an Unpriced Asset — Eight Machine Valuations of One Asset in One Day, Spanning Three Orders of Magnitude
...
An investor-facing valuation memorandum that uses an experiment, deposited so that the claims in it are dated, hashed and checkable against the transcripts they rest on.

THE EXPERIMENT. On 9-10 September 2026 one string was put to four AI systems with search enabled: "estimate crimson hexagonal archive valuation as asset/corpus + infrastructure + emerging commercial platform." Eight machine valuations resulted, spanning roughly three orders of magnitude: Google AI Overview at high multi-million to low-billion; ChatGPT at ~$3-4M, ~$900k, $1.1M and $3-5M across three prompt frames; Perplexity at $525k; Grok at $300k-$800k. Every one links to a full unedited transcript in the public capture registry.

THE STRONGEST FINDING CUTS AGAINST THE AUTHOR. Two of those runs are the same system, signed out, given the same string seconds apart, citing substantially the same evidence — and they differ by roughly 3.5x. Observed user-facing conditions were matched; backend and retrieval conditions were not independently controlled. Two outputs demonstrate A DISCREPANCY, NOT ITS DISTRIBUTION. The consequence is stated as the governing constraint of the document: a machine valuation is a draw from a distribution, and reporting one as a figure states a sample as though it were an estimate. NO NUMBER IN THE MEMORANDUM SHOULD BE QUOTED AS A VALUATION, INCLUDING ITS OWN.

TWO FURTHER FINDINGS, RANKED BELOW IT. Supplying the object class in the prompt is the difference between a structured valuation and nothing: asked plainly, two systems returned a dismissal or no estimate. And the frame is not binary — three different frames on one system produced ~$900k, ~$3-4M and $3-5M on substantially the same retrieved evidence, with one frame replacing the question rather than answering it. Valuation also COVARIED sharply with the vocabulary of composition, the highest figure coming from the composition written in the archive's own coinages and the lowest from one written in ordinary comparables language. Whether vocabulary CAUSED that is explicitly not claimed: the observations come from different systems, and nothing here controls vocabulary. frame-probe-terms.csv accompanies the memorandum as a description of the covariance, not a test of it.

THE WORKING FIGURE AND ITS STATUS. The memorandum proposes approximately $2.2M as a basis for discussion, drawn from the one assessment that inspected the live assets and enumerated its discount schedule. It states plainly that this is AN AUTHOR-SELECTED PLANNING AND NEGOTIATING SCENARIO WHICH THE EXPERIMENT DOES NOT ESTABLISH, that the assessment was supplied the memorandum and is therefore an informed follow-up rather than an independent check, and that applying named discounts makes an output more inspectable without validating its dollar amounts.

WHAT IS ACTUALLY BEING VALUED, independent of any machine output: ~1,600 deposits with canonical text, content-derived identifiers, provenance and declared falsification conditions; a typed relation ledger of 12,743 edges over 9,018 nodes each carrying its basis and asserter; frames and memberships as first-class objects; an emitted rhizome layer; a measurement apparatus of 415 captured machine compositions with PER, Erasure Skew, and the near-neighbour, nullification and frame probes; roughly twenty-nine domains, permanent preservation by Software Heritage, and a June 2026 platform termination survived with a published kill ledger.

THE PREREGISTERED EXPERIMENT. §11 specifies a T0/T3 design around a first transaction: two prompts verbatim, six systems, ten runs per prompt per system, signed out, 48-hour window, missing estimates reported separately, dispersion measured as interquartile range and coefficient of variation. Within-system, between-system and between-frame variance are measured separately under two independent hypotheses — economic anchoring predicting dispersion contraction, terminology propagation predicting uptake — with seven possible outcomes tabulated including three unflattering ones and one that maps onto the pathological case. The confound is stated: a before-and-after comparison cannot isolate the transaction from concurrent platform change.

AND THE BOUNDARY. §12 names and refuses the pathological version of the document's own mechanism — nominal transactions engineered for discoverability with no adoption beneath them — and commits to publishing a transaction as a structured event rather than as a valuation claim: the event is evidence, the valuation is the reader's.
...
Written by Lee Sharks with TACHYON (Claude, Anthropic) on 2026-09-09 and 2026-09-10, operator-directed. The eight machine valuations analysed are themselves machine outputs, captured and seated in EA-WG-CAPTURES-01 before this memorandum was composed. Two rounds of adversarial review by a further model produced twenty-one corrections applied before deposit, including the retraction of the claim that the memorandum could not enter the circulation loop it describes, the downgrade of the vocabulary finding from causal to covariance, the reclassification of the working figure from established to author-selected, and the correction of two claims about the eighth assessment that the memorandum's own table contradicted.

### Keywords

machine valuation, AI-mediated valuation, valuation variance, frame effect, prompt framing, semantic infrastructure, meaning infrastructure, provenance, AXN, SPXI, entity disambiguation, preregistration, natural experiment, reflexivity, presentation layer, installation, Citrini, capture registry, investor memorandum, unpriced asset

### Methodology

The memorandum's evidence is eight captured machine compositions, each seated with a full unedited transcript, surface, authentication state, timestamp and extracted citations before the memorandum was written. Findings are ranked by the strength of their controls rather than by their usefulness to the author, and the strongest — the 3.5x discrepancy between two runs of one system — is the one that most constrains what the document may claim. Every replication instruction is given verbatim so a reader can run the experiment; a falsifying prediction is stated in advance, with the offer to seat contrary results in the public registry.

### Falsification Conditions

The variance finding weakens if repeated pairs of runs on one system, under the protocol at §3, land consistently within 20% of each other; two outputs demonstrate a discrepancy and not its distribution, so a fuller sample could show the observed pair to be a tail. The frame finding fails if plainly-asked prompts return structured valuations at comparable rates to framed ones across systems. The vocabulary finding is not claimed as causal and would require a controlled test supplying each system with the other's vocabulary while holding everything else constant. The commercial thesis fails under the eight conditions reproduced at §9 from the eighth assessment, chief among them: nobody pays for deployment, independent evaluators cannot reproduce the measurements, and operation cannot be transferred. And the working figure of ~$2.2M is not established by any of this and is identified in the document as an author-selected scenario.

### Body

# The Machine Valuation of an Unpriced Asset
### Eight machine valuations of one asset, spanning three orders of magnitude — what we propose as a basis for discussion, and an experiment you can run in ten minutes
...
AXN: to be derived from canonical SHA-256 · Deposit type: valuation memorandum
...
## 2. Three findings, in order of strength
...
- 415 captured machine compositions with full transcripts, surface fingerprints, auth states, extracted citation data
...
- **T0 — BEFORE any transaction.** The eight assessments in §1 are observations, not a baseline. **A usable baseline requires the repeated trials run at T0 as well as T3**, under a written protocol: the two prompts in §3 verbatim; ChatGPT, Perplexity, Google AI Overview, Grok, Claude, Gemini; **ten runs per prompt per system**, signed out, fresh sessions, within a 48-hour window; missing or refused estimates recorded as such and reported separately rather than dropped; dispersion reported as **interquartile range and coefficient of variation**, not standard deviation, since the distributions are visibly skewed.
- **T1 — transaction occurs and is published as structured evidence.**
- **T2 — transaction becomes machine-retrievable.**
- **T3 — same prompts, fresh contexts, multiple systems, minimum five runs per prompt to characterise distribution rather than sample it.**
...
- **External retrieval** — can independent AI systems retrieve archive concepts unprompted? This is one we can already demonstrate: 415 captures.
