...
THIRTEEN INVARIANTS, OF WHICH THREE ARE NEW IN THIS VERSION. Unknown is not zero. Monetary settlement is recorded, not denied. Monetary settlement does not silently become semantic settlement. Omission is not automatically erasure. Provenance available is distinct from provenance required. Commensuration is not automatically predation. Selection pressure must be evidenced rather than asserted of any distinction that happens to be qualitative. Translation is not inverse decompression. Remainder remains explicit. The audit itself remains revisable. And now: no scalar audit score, because an instrument built to refuse a magnitude standing in for a structure may not produce one about itself; closure and integrity are recorded separately, never collapsed into one value; and the auditor's relation to the object and to the inscription is recorded, since an audit performed by the author of what it audits is weaker and the weakness belongs in the ledger rather than in a reader's inference.

FOUR SECTIONS THAT THE FIRST WORKED SPECIMEN FORCED. §4.1 requires every variable entering a formula to declare the owner of its denominator, because a coefficient whose denominator is the observer's own effort moves when the instrument moves and the world does not, and a valuation containing one can be changed by working harder without learning anything. §14a adds the inscription-integrity audit with nine controlled defect classes, since the settlement audit asks what a scalar closed and not whether the scalar was legal, and an inscription can preserve every pointer, claim no finality, satisfy every other invariant and still rest on arithmetic that does not hold. §15.1 makes classification a pair, because the first specimen occupied a cell the one-axis witness/predatory definition did not have — a dimensionally incoherent product carrying a proper audit pointer classifies as witness on closure and is unsound on an axis that did not then exist. §17a adds the audit score vector: six rates and one count, each reported with its numerator and denominator visible, never summed.

AND IT CARRIES AN ERRATUM IT MADE. §17b reconciles the three published arities of the same operation and records that #1618 (AXN:06AA) had described this ledger as eleven-element, which counted a draft schema's sections rather than the canonical tuple. The correction was applied to that deposit as v0.4.1 on 2026-09-16, marked in its canonical text rather than made silently, with an errata record and a re-anchored hash.
...
Specification written, then implemented, then revised by what the implementation could not express. A JSON Schema was authored against the prior version; the first worked specimen was filled against it and validated; the four sections and three invariants added in this version are each traceable to a field the fill required and the schema lacked. Arity reconciled against the two other published forms of the same operation, with the discrepancy corrected on the other deposit rather than absorbed here.

### Falsification Conditions

Six, stated in §21. The two-axis classification fails if closure and integrity covary necessarily, and the first specimen already occupies a cell that collapse would forbid. The settlement-endogeneity premise, inherited from the translation protocol, fails if a monetary grammar can be exhibited whose ordinary operation leaves the account open without an external structure. Invariant 7 fails if selection pressure is unevidenceable in principle rather than merely unevidenced in a given audit. The scoring vector fails if its six rates prove to measure one underlying quantity, in which case the prohibition on summing is a scruple rather than a principle. And the protocol fails as a whole if a sustained run of audits produces classifications that no independent auditor reproduces from the same ledger, which would show the fields to be a vocabulary rather than an instrument. The protocol may not be vindicated by the failure of any particular valuation, nor by a rich enumeration of losses in a single audit: a ledger that found something wrong with every inscription it examined would be measuring its own disposition.
...
\[
\text{What is }X\text{ worth in dollars?}
\]
...
\[
\boxed{
m\in\operatorname{MSAL}(X)
\qquad\text{but}\qquad
m\neq\operatorname{MSAL}(X).
}
\]
...
- \(X_E\) = evidence-supported pre-monetary object;
- \(m\) = monetary inscription;
- \(\delta\) = valuation purpose and scope;
- \(\sim_\delta\) = commensuration contract;
- \(G_M\) = monetary grammar invoked;
- \(K_M\) = capacities enabled by monetary inscription;
- \(\Delta_D^M\) = distinctions not required by the monetary representation;
- \(\Delta_C^M\) = distinctions that lose standing against monetary settlement;
- \(R_M\) = relations transformed by monetary grammar;
- \(P_M\) = provenance condition before and after inscription;
- \(B_M\) = bearing / cost condition;
- \(C_M\) = settlement operation and closure state;
...
\[
\boxed{
\text{What had to become “the same enough” for the arithmetic to be legal?}
}
\]
...
\{
d\in D_E(X):
d\text{ lacks standing to veto }C_M
\}.
}
\]
...
\[
\text{labor}\rightarrow\text{wage}
\]

\[
\text{harm}\rightarrow\text{damages}
\]

\[
\text{creative work}\rightarrow\text{asset}
\]

\[
\text{future possibility}\rightarrow\text{option value}.
\]
...
\[
\boxed{
\text{not represented in the monetary inscription}
}
\]
...
\boxed{
\operatorname{settled}_M
\text{ is a valid state inside }G_M.
}
\]
...
\boxed{
\rho_M
\text{ is never silently assigned }0.
}
\]
...
\[
\boxed{
\neg\operatorname{Final}(\operatorname{MSAL}).
}
\]
...
\[
\boxed{
\text{MSAL-0001 — Temporal Projection / Sappho–Carson}.
}
\]
...
\[
\boxed{
\text{What did the monetary grammar do to the object in order to make the scalar settle?}
}
\]
