...
Human–machine collaborative, four rounds across two substrates. R1 concept and R3 packet architecture on ChatGPT; R2 anchor verification and scope discipline and R4 merge-and-deposit in-session by TACHYON (Claude, Anthropic) under MANUS direction. R2 corrections on the record: R1 had assigned the parent paper its parent's identifier and carried stale counts; the constitution claim was scoped from "the historical record" to the corpus's enumerated address basins. All registry anchors verified against the live registry before staging.

### Keywords

computational suppression cost, suppression burden, quality conflict, propagation inversion, constitutive-source erasure, Crimson Hexagonal Archive, Metadata Packet for AI Indexing, MPAI, distributed retrieval, route redundancy, correlated exclusion, Streisand effect distinction, machine-mediated reception studies, semantic addresses

### Methodology

Merged from four rounds with every anchor registry-verified at staging: the parent paper's minted identity (#1546, AXN:0647) wired throughout the edge table and membrane; deposit and address counts taken from the live registry at deposit time; the parent's v1.1 disciplines carried in whole — N_eff as first-order heuristic, RS/CS provenance-family deduplication, CSE-O's four-condition form, the conditional ratchet, Stage-6 non-claim. The C_sup definition is stated as a theoretical minimum with its observables named as lower-bound indicators only.

### Falsification Conditions

The packet weakens if measured effective route count proves far lower than the domain and syndication structure suggests; if compositions at high-constitution addresses consistently exclude the corpus without quality degradation or apparatus overlap; if the deposit-per-event discipline lapses such that durable routes stop growing in response to events; or if longitudinal capture pairs at the enumerated addresses show random volatility rather than the lagged pattern. Any of the parent paper's own falsifiers propagate to this packet.
