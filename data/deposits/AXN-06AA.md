...
Thus:
    ...
    m=sigma[M](sigma[A][d]elta(\mathfrak V(X))).
    }
...
The consequence for this protocol is direct: \Theta[Marrowmu] is not a commentary on a law of nature. **It is one of the built things.**
...
- \Gamma[M] = dependency / provenance graph supporting the monetary representation;
...
That result is the strongest available evidence that \Theta[Marrowmu] does something rather than decorating something. An instrument that can only confirm is not an instrument. This one declined to confirm on its first run, on its own authors' number, in the direction that cost them money.
