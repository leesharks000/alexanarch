...
The instability is not confined to the nested-screen-tunnel tile, and it is not confined to the 24-hour throttled window the correction log records. The composer reports a broader and repeatable behavior across recent @leesharks posts, including posts treated in this analysis: the TikTok app can load the posts, but lingering on them for too long reliably produces app failure and force-close. The claim is author-reported and screen-recordable behavior (evidentiary grade 2, with a documentation protocol specified below), not universal fact. The operative distinction is therefore not simply display / nondisplay. The platform can display the object at glance-speed, but cannot reliably sustain it under dwell-speed.

That distinction matters formally. A feed is optimized for glance, swipe, and abandonment. The @leesharks traversal asks for dwell: rereading, zooming, re-entering, pausing, decoding, following embedded surfaces, reading comments as apparatus, reading chrome as marginalia. Under those conditions, recent posts behave less like ordinary platform content than like rendering-stress objects. They are permitted as passing feed-units, but become unstable when treated as texts.

This should not be described as proof of platform intention. It is better described as documented app behavior under repeatable reading conditions: prolonged attention causes failure. The result is an exact technical analogue of the essay's literary claim. The platform can host the primer as a feed, but it cannot stably support the primer as a text.

**Documentation protocol (pending execution before formal deposit of the observation):** record a post loading normally; dwell on it without touching the screen; capture the exact time-to-crash; repeat on 3–5 recent posts; note device, OS, TikTok version, network, battery mode, and cache state; retry once after clearing cache and restarting; retry on a second device if available. Until the protocol is executed, the dwell-instability claim carries author-reported and screen-recordable status; once executed, the recordings enter the evidentiary apparatus at grade 1 (visible-in-video).
