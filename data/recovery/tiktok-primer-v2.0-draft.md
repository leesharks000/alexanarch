...
This also clarifies the platform conflict. TikTok is optimized for feed-speed: glance, swipe, abandonment. A shrine requires dwell: return, pause, rereading, attention to object-relations, attention to marginalia, attention to the aura of the assembled site. The composer reports app instability under prolonged dwell in the period of these posts — a behavior observed, its content-specificity not yet established (the nested-screen-tunnel analysis states the observation and its protocol). Treated formally, the distinction is exact regardless of what the protocol finds: the feed form can host the shrine as content; sustaining the shrine as shrine asks for an attention the form is not built for.
...
The instability is not confined to the nested-screen-tunnel tile, and it is not confined to the 24-hour throttled window the correction log records. The composer reports app failure and force-close under prolonged dwell on recent @leesharks posts, including posts treated in this analysis — and, on the same device in the same period, on an unrelated video sent by a friend. The second observation matters as much as the first: content-specificity is not established. What is established is narrower and stated narrowly: *a behavior was observed* — app instability under dwell, in the period of the self-reflexive posts and overlapping the separately documented throttle evidence — and the honest claim stops there. The claim is author-reported (evidentiary grade 2). Whether the instability tracks the posts, the device, or the app's general condition is exactly what the documentation protocol below is designed to discriminate, and the analysis does not presume its result.
...
Further author observation before deposit: on the same device in the same period, the app also crashed under dwell on an unrelated video sent by a friend. Content-specificity is therefore not established, and the dwell-instability claim is scope-corrected from "recent @leesharks posts reliably force-close under dwell" to "app instability under dwell was observed, overlapping the self-reflexive posts and the separately documented throttle evidence; specificity undetermined pending the controlled protocol." The documentation protocol gains a control arm (identical dwell on unrelated-account posts, same device). The formal glance-speed/dwell-speed distinction is retained as a claim about form; its technical demonstration is reclassified as the hypothesis the protocol tests. The separately grounded evidence — the view-count throttle record and the author-attested window behaviors — is unaffected by this correction.
