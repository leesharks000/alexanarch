...
> **INTERNAL SESSIONS — ENRICHMENT IS PART OF THE DEPOSIT.** Every internal deposit must finish by regenerating the citation graph, entity index, and wiki entries — not just the record page and browse surfaces. A deposit is not complete when it appears at `/s/records/<N>/`; it is complete when other deposits know how to find it and the citation graph reflects its incoming and outgoing links. Concretely, after `deposit_pipeline.py` finishes and before you close the session, run the following mechanical passes in this order and commit the result:
>
> 1. `python3 scripts/citation_extractor.py` — updates `data/citation-graph.json` (internal AXN / EA-ID / #N / legacy Zenodo DOI edges)
> 2. `python3 scripts/extract_citations_external.py` — updates `data/citation-graph-external.json`, `data/external-works.json`, and `data/worklists/refsec-parse-queue.json` (external DOIs, URLs, arXiv, ISBNs, and freeform reference-section segmentation)
> 3. `python3 scripts/concept_backlink.py` — updates entity backlinks so `defined_in` fields resolve
> 4. Any newly-`pending` entries in `refsec-parse-queue.json` are parsed **in-session** (No-Double-Draw: TACHYON parses freeform reference entries directly, never via API)
>
> Wiki articles are generated by the mint workflow's Anthropic API path for transport-A deposits only. For internal (transport D) deposits, the wiki article is authored by the depositing MANUS/TACHYON in-session and written directly into the registry entry's `wiki_article` field before the deposit-time surface regeneration. Do not leave `wiki_article` empty; do not defer it. The archive's browse surfaces and cross-links depend on every deposit carrying a wiki entry from the moment it is minted.
>
> Skipping any of these steps leaves the archive in a partial state that later deposits inherit. Deposit-time enrichment is not optional infrastructure work; it is what makes the deposit visible to the rest of the archive.
