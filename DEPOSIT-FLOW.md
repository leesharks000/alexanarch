...
## STAMPING DEPOSITS (symbolon, added 2026-08-24)

Every deposit should be symbolon-stamped after its mint deploys: `python3 scripts/stamp_deposit.py --deposit-number <N>`. The script implements the axnidentifiers.org/stamp/ algorithm (AXN-SYMBOLON-SPEC v0.3, addressed/chiastic, generation 2) against the deposit's CANONICAL bytes: axn0 = the registry hash (the sealed core), the stamped copy is a distribution artifact carrying the full AXN in a suffix comment block, axn1 witnesses that circulating form, and the Seed A sidecar is registered through the public endpoint with the sealed core as the stored file — the endpoint refuses any store that does not hash to axn0, so the archive holds only cores while stamped copies travel. Idempotent (skips already-witnessed hashes); the sealed core is resolved BY HASH across the canonical surfaces — facsimile deposits (body_status: primary-source-facsimile, e.g. #1539) deliberately serve page images at full_text_path while data/deposits/AXN-{hex}.md preserves the mint bytes, and the stamper seals whichever surface hashes to the registry hash, refusing only when none does. One stamp = one symbolon-service commit = one deploy, fine at mint cadence. STANDING POLICY (MANUS, 2026-08-24): stamping is forward-only — every deposit from #1539 onward is stamped at mint; the back catalog stays unstamped for now, by decision rather than omission. Revisiting that requires a fresh ruling on local allocation semantics before anyone loops the endpoint. First stamped deposit: #1540 → AXN:063C.GOVERNANCE.🍀🃏⊗⌛🙏□, witnessed-verified.

## KNOWN PRESENTATION ISSUE — ATTACHMENT PLACEMENT (noted 2026-08-24, deposit #1540)

The mint composes repo-hosted text attachments as inline ingests at the TOP of the canonical file, before the article body, so the record page's Full Text opens with the attachment rather than the work. The desired presentation: article first, attachments after the body — ideally rendered as clickable download entries below Full Text rather than inline. Two constraints on any fix: (1) for already-minted deposits the ingest occupies the AXN derivation window (content[:5000]), so canonical files must NOT be reordered post-mint — the fix for existing records LANDED 2026-08-24 in the renderer (`wire_deposit._strip_inline_ingests`, called from `regenerate_static_page`): declared-attachment ingest segments are excised from the DISPLAYED Full Text and JSON-LD articleBody only — located by the mint's own heading template per registry-declared attachment, returning the body unchanged if any declared heading is absent — while the Held-artifacts block below Full Text serves the files as downloads with size and sha256 (its host filter now accepts both www and bare alexanarch.org URLs); canonical bytes, MD download, and PDF untouched; (2) for future mints, `mint_deposit.py` may compose ingests after the body, which changes only new derivations. Until either lands, depositors should expect inline ingests to precede the article and may prefer citing attachment URLs without repo-hosting text files they do not want ingested inline. Separately, for the record: the wrapped prose found in #1540's SPXI kernel was NOT a pipeline defect — it entered in thread-side composition and was hand-repaired per Rule Zero (kernel joined to one logical line, whitespace-only change, AXN window byte-identical, registry hash updated); `unwrap_deposit.py` correctly refused the block as indented structure both before and after, which is the conservatism working as designed.
